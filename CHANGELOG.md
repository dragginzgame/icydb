...
- `0.65.5` finishes moving the distinct pagination continuation checks into the live executor owner suite and fixes a structured-field bug where nested custom records could collapse to `null`, so values like `Profile::default()` now round-trip as full structured data instead of being lost on the new storage/projection path.
