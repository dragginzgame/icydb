...
- `0.150.3` routes runtime field-path cursor-anchor and forward/unique write index key construction through accepted `SchemaInfo` index contracts, using accepted index ordinals, accepted row-layout slots, accepted store paths, and accepted uniqueness for field-path keys and preflight membership decoding while keeping expression-index key construction on the explicit generated deferred lane.
