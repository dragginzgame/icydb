...
- `0.59.2` keeps the de-monomorphization cleanup going by moving field-aggregate streaming and final typed response assembly onto structural or outer-edge boundaries, collapsing duplicated persisted-row decode paths, and remeasuring both canister harnesses in `debug` and `wasm-release`.
