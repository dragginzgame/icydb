...
- `0.61.3` keeps flattening the simple-entity runtime floor by first removing the old per-entity `LoadExecutor<E>::execute` hot-path wrapper and then splitting generated SQL query and explain dispatch onto narrower callbacks, which drops the `one_simple -> ten_simple` shrunk slope again from `729,703` to `716,314` bytes.
