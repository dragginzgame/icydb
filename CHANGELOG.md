...
- `0.261.6` removes redundant row and prefix copies from journal writes, recovery and child enumeration while preserving checked cardinality updates.
- `0.261.5` fixes false recovery-pending errors, removes redundant key copying and decoding, and corrects indexed-scalar measurement boundaries.
...
- `0.259.7` adds end-to-end qualification of application batch reads, validation, writes and request chunking without changing database behavior.
- `0.259.6` simplifies write preparation and commit completion while preserving validation and recovery.
...
- `0.253.3` allows subsequent migrations after completion or abort and protects
  active migrations from unrelated schema changes while preserving recovery.
...
- `0.253.0` freezes nested-relation limits, hard-cuts relation identity, and
  ...
  and release validation now bounds core-test parallelism on high-core hosts.
...
- `0.252.8` seals one parse-once SQL dispatch across read, write, and DDL flows,
  ...
  canonical owners. Release and CI workspace tests now cap host-independent
  parallelism to prevent maximum-bound fixtures from exhausting memory. The
  cleanup reduces write cost and removes 5,391 raw Wasm bytes with
  byte-identical Candid.
...
- `0.249.6` exposes the owned prepared-row and live-page boundary needed by
  trusted/public downstream adapters, preserving the 0.249.5 hard cut without
  restoring row-at-a-time projection.
...
- `0.249.4` hard-cuts filters to eight canonical expression families and
  ...
  maintained semantics retain about 30 KiB of measured project-instance code
  headroom before the enum-only change.
...
- `0.241.0` narrows exact-count and prepared aggregate helpers to their actual
  same-file owners without changing SQL behavior or execution flow.

## [0.240.x] ⚡ - 2026-08-25 - Exact Global-Aggregate Execution

Detailed notes: [docs/changelog/0.240.md](docs/changelog/0.240.md)

- `0.240.1` lets a bare `COUNT(DISTINCT non_nullable_int32_field)` use exact
  synchronized leading-index metadata, cutting the measured 2,048-row query
  from 120.7M instructions to 0.47M with fail-closed metadata and atomic budget
  handling, without adding persisted or cache state.
...
- `0.237.2` lets one-sided compound-index ranges begin at their encoded edge
  ...
  uses one pinned, runner-owned PocketIC server for its parallel fixture pool.
...
- `0.237.0` broadens exact SQL performance evidence across count, extrema,
  ...
  covering secondary ordered endpoints by 54.0M–54.2M, and saves 40.5M on the
  measured 17-key exact-prefix count without raising the shared page cap, while
  preventing untracked files from contaminating release validation.
...
- `0.235.1` restores SQL and dynamic predicates for accepted `#[newtype]`
  ...
  while retaining nominal write admission and fail-closed structural composites.
...
- `0.222.2` bounds and reports grouped retained state, speeds aggregate
  ...
  which must handle the added `peak_estimated_state_bytes` field.
...
- `0.221.9` removes duplicate row-payload reads and adds opt-in bounded request
  diagnostics that reveal N+1 query patterns and useful compound indexes
  without exposing query literals or stored keys.
...
- `0.221.3` bounds physical work across maintained read routes, upgrades host
  ...
  artifacts without changing successful results, Candid, or persisted formats.

- `0.221.2` makes repeated reads safer and cheaper through bounded
  parameterized plan reuse, complete typed filter-literal rehydration, and
  monotonic hard execution budgets on the planner-free exact-key path, while
  leaving successful results, Candid, and persisted formats unchanged.
...
- `0.215.2` adds one bounded atomic same-entity structural batch and raises
  generated canisters to `ic-memory 0.12.3`, retaining atomic recovery and
  unified native runtime ownership without changing stable formats.
...
- `0.213.41` finalizes current-name schema authoring with typed timestamps and
  exact existing-head named-type matching, deleting the residual implicit
  metadata-rename machinery.
...
- `0.213.33` completes the model consumer hard cut, moving maintained callers
  ...
  the cutover with atomic bounded SQL responses, mixed-store schema
  application and recovery, and accepted-authority SQL integrity routing.
...
- `0.212.0` establishes one accepted-native Quick/Deep integrity core with
  ...
  integrity report; authorized typed and SQL entrypoints remain follow-up work.
...
- `0.208.1` completes the pre-1.0 exact-composite hard cut by enforcing complete
  ...
  schema introspection, including canonical nested-leaf order and recursively
  validated relation roles, with inconsistent contracts failing closed.
...
Detailed notes: [docs/changelog/0.207.md](docs/changelog/0.207.md)

- `0.207.0` makes user-index schema publication and recovery marker-first and
  redo-only through one bounded domain replacement, removing operation-specific
  rollback and the obsolete `IndexState::Dropping` lifecycle.
...
- `0.201.2` moves visitor, runtime-value, and key contracts to their owners
  ...
  and consumers of raw diagnostic numbers must adopt the current contracts.
...
- `0.199.31` continues the technical-debt audit line by splitting metrics
  counter mutation ownership and aligning the core crate with Rust 1.97 Clippy
  without changing runtime behavior.

- `0.199.30` continues the technical-debt audit line by splitting concrete
  metrics event dispatch and typed instrumentation span helpers out of the
  metrics sink facade without changing metrics behavior.
...
- `0.199.27` continues the technical-debt audit line by splitting
  read-admission policy, planner summary extraction, rendering, and tests into
  separate owners without changing read-admission behavior.
...
- `0.199.11` continues the technical-debt audit line by splitting fluent load
  ...
  remaining terminal method list without changing terminal behavior.
...
- `0.195.0` starts deferred access physicalization by keeping large indexed
  ...
  reducing sparse `IN (...)` page planning cost without changing route, cache,
  cursor, or persisted-format semantics.
...
- `0.187.8` continues recoverable invariant cleanup by routing scalar predicate
  ...
  through fail-closed, parse, or typed key errors instead of traps.
...
- `0.187.0` starts the second query-engine audit by guarding metadata-count
  authority, SQL write bounds, and scalar execution-input convergence while
  validating generated SQL canister parity and converting audited query/write
  invariant traps to recoverable behavior.
...
- `0.184.33` continues the query-engine audit by making global aggregate
  ...
  without changing aggregate semantics.
...
- `0.184.30` continues the query-engine audit by consolidating SQL execute
  diagnostics attribution for global aggregates and SELECT without changing
  query semantics.

- `0.184.29` continues the query-engine audit by consolidating DELETE executor
  runtime shells and SQL INSERT/UPDATE mutation execution handoff without
  changing write semantics.

- `0.184.28` continues the query-engine audit by consolidating SQL write row
  batching, scalar page windowing, and metadata-backed count/existence
  terminal execution without changing query or write semantics.
...
- `0.184.23` continues the query-engine audit by consolidating large `IN`
  ...
  changing SQL or predicate semantics.
...
- `0.184.13` continues the query-engine audit by consolidating grouped
  ...
  reducing duplicate planner/session shaping without changing query semantics.
...
- `0.184.12` continues the query-engine audit by compacting large SQL
  ...
  materialized sorting, collapsing sparse covered prefix-set pages after
  empty-prefix pruning, and adding aggregate/materialization diagnostics for
...
- `0.184.5` continues the query-engine audit by carrying analyzed global
  ...
  strict-prefilter EXPLAIN stage observability on one route-owned contract.
...
- `0.184.2` continues the query-engine audit by reusing aggregate expression
  ...
  candidates, and surfacing residual-filter shape in EXPLAIN diagnostics.
...
- `0.183.20` trims fluent branch-set full-entity page reads by reusing
  ...
  fluent branch-set perf coverage for cold and warmed query-plan paths.
...
- `0.183.17` trims cold SQL parse cost for literal-heavy membership filters by
  ...
  and avoiding unnecessary WHERE postfix expression clones.
...
- `0.183.2` fixes covering page-window regressions around residual predicates,
  primary-key ordering, and key-only `LIMIT` scans, then adds
  production-shaped branch-set audit coverage against an over-cap fallback.
...
- `0.182.17` trims blob projection overhead by moving owned SQL output values
  without extra blob clones and adds blob payload attribution to the SQL perf
  audits.
...
- `0.182.7` removes the remaining internal direct-stable runtime backend and
  ...
  journaled cached-stable recovery.
...
- `0.181.11` makes validated public SQL `UPDATE RETURNING` fail closed when
  ...
  schema-transition cleanup branches.

- `0.181.10` adds generated SQL update source-boundary proof and narrows
  aggregate projection plus generated persisted-row ownership cleanup.

- `0.181.9` adds live generated `UPDATE RETURNING` canister coverage across
  primary-key and bounded star projections while simplifying scalar aggregate
...
- `0.179.6` continues the repeated-audit cleanup line by routing accepted
  ...
  SQL to explicit opt-in, trimming unused Candid derives, and refreshing the
...
- `0.178.6` moves SQL DDL field-default, nullability, and rename candidate
  ...
  ordering and existing bind diagnostics.
...
- `0.177.1` tightens schema-version admission diagnostics with compared
  ...
  out of storage publication, preserving DDL-owned catalog extensions and
  backfilling remaining fixture declarations.
...
- `0.170.3` classifies public write commit plans as durable-only, live-only,
  or mixed durable/live from affected store capabilities.
...
- `0.168.6` closes the runtime store-backend boundary proof matrix with
  source-audit evidence and focused validation for generated wiring,
  allocation identity, codec preservation, commit/recovery behavior, and
...
- `0.166.3` adds public fail-closed proof for unsupported SQL DDL/query edges:
  ...
  post-aggregate SQL while locking invalid grouped projection shapes closed.
...
- `0.162.16` moves remaining planner, cursor, covering, row-decode, and
  ...
  while keeping generated-model fallbacks isolated to model-only/test paths.
...
- `0.162.6` opens composite primary-key declarations and generated key
  contracts while keeping scalar-only execution paths explicit and
  fail-closed.
...
- `0.162.3` moves derive and key-taxonomy internals onto ordered
  primary-key metadata, including fixed-capacity composite value and generated
  key-type scaffolding while keeping composite declarations gated.

- `0.162.2` adds runtime `PrimaryKeyModel` metadata so generated entity models
  can carry ordered primary-key fields before composite storage behavior
  changes.

- `0.162.1` hard-cuts generated primary-key attributes to
  `pk(fields = ["id"])` and moves schema primary-key metadata to an ordered
  field-list shape while scalar execution remains unchanged.
...
- `0.161.4` completes audit-driven hygiene across build/config crates,
  facade boundaries, and downstream audit fixtures without changing runtime
  behavior.
...
- `0.158.9` adds `ALTER TABLE ... DROP COLUMN ...` parsing, CLI/canister DDL
  ...
  index-dependent columns before ordinary DDL-owned fields fail closed with a
  retained-slot field-removal policy error.

  ```
  ALTER TABLE Character ADD COLUMN nickname text;
  ALTER TABLE Character DROP COLUMN IF EXISTS old_nickname;
  ALTER TABLE Character DROP COLUMN nickname;
  DESCRIBE Character;
  ```
...
- `0.157.2` adds metadata-only reconciliation for hard-cut generated index
  ...
  and predicate contracts are unchanged, without rebuilding physical index
  entries.
...
- `0.154.2` consolidates the accepted-snapshot reconciliation tail shared by metadata-only schema-store reconciliation and runtime-store startup reconciliation while keeping physical schema mutation execution isolated to the runtime startup path.

- `0.154.1` fixes schema default generation so every primitive field can emit accepted persisted database-default payloads through the existing field-codec lanes, including Decimal, Int/Nat big integers, 128-bit integers, identity/key primitives, Blob, Unit, temporal primitives, and optional single-value fields with non-null literal defaults; hard-cuts unsigned numeric runtime and persisted metadata names from `Uint*` to `Nat*`; and runs the supported non-unique field-path index-add path during runtime startup reconciliation against accepted row contracts when the physical index store is empty.
...
- `0.153.11` adds closeout guard/docs for the schema rebuild runner minor, locking field-path runner modules to accepted runner input, execution plans, and rebuild targets without generated `EntityModel`/`IndexModel` authority, recording the completed 0.153 field-path runner foundation versus the intentionally deferred 0.154 reconciliation/recovery work, and tightening the 0.154 developer-stability design around one supported non-unique field-path index add path with explicit publication ordering and diagnostics.
- `0.153.10` tightens field-path runner publication readiness so snapshot publication handoff no longer makes generic runner diagnostics publishable until the physical `IndexStore` is promoted to ready, adds an explicit physical-store publication phase/blocker with source-boundary guard coverage, and adds the `0.154` developer-stability design for shipping accepted-schema-backed database testing without broad SQL DDL.
- `0.153.9` adds typed field-path runner failure diagnostics with rollback reports, so isolated physical validation failures after staged writes roll back through the captured write-batch rollback plan and return counted rollback actions without invalidating runtime state or publishing snapshots.
- `0.153.8` adds the physical field-path index-store publication step after validated runner work and accepted snapshot publication handoff, promoting the isolated rebuilt `IndexStore` from building to ready only after the ordered validation/invalidation/publication boundary completes, and adds a target-mismatch guard that rejects the runner before physical work starts.
- `0.153.7` adds a typed field-path index rebuild runner that accepts only the schema-owned field-path execution plan shape, stages accepted row data, writes and validates an isolated `IndexStore`, invalidates runtime schema epochs, and hands the accepted-after snapshot to the publication sink in one ordered report while rejecting non-field-path plans before physical work starts.
- `0.153.6` adds runtime invalidation and accepted snapshot publication handoff boundaries for validated staged field-path index stores, binding isolated physical validation to accepted before/after schema epochs through mockable invalidation and publication sinks, reporting invalidation and snapshot-publication phases, keeping the real schema-store write behind an explicit sink boundary, and splitting the schema mutation implementation into focused runner, field-path, and test modules.
...
- `0.152.7` adds schema-owned runner capability derivation, admission checks, runner preflight, publication-boundary preflight checks, and closeout guard/docs for mutation execution plans, so future physical rebuild/cleanup runners can be checked against accepted field-path rebuild, expression-index rebuild, secondary-index cleanup, validation, invalidation, and rewrite capabilities before publication policy is widened while SQL DDL and physical rebuild execution stay deferred.
- `0.152.6` adds a schema-owned mutation execution boundary that derives publishable-now, physical-runner-required, and unsupported execution plans from rebuild classifications, exposes a compact execution gate for future callers, schedules accepted index rebuild/drop work before validation and runtime invalidation while keeping rebuild-required publication fail-closed, and splits release bump, stage, commit, tag, and push actions into explicit Make targets.
- `0.152.5` adds accepted secondary-index drop cleanup targets by lowering non-unique secondary-index drop requests from accepted persisted index snapshots into schema-owned cleanup targets that carry accepted index ordinal, name, store path, uniqueness, and predicate SQL metadata, while keeping publication blocked until physical cleanup execution exists.
- `0.152.4` adds accepted deterministic expression-index rebuild targets by lowering expression index mutation requests from accepted persisted index snapshots into schema-owned targets that preserve mixed key-item order, accepted source slots, expression op, input/output kinds, and canonical expression text, while keeping publication blocked until physical rebuild execution exists.
- `0.152.3` starts accepted field-path index rebuild targeting by lowering non-unique field-path index mutation requests from accepted persisted index snapshots into schema-owned rebuild targets that carry store identity, predicate display metadata, and accepted row-layout key slots, while keeping publication blocked until physical rebuild execution exists.
- `0.152.2` adds schema-owned rebuild planning contracts and the internal `SchemaMutationRequest` lowering surface, so accepted snapshot/proposal diffs and future DDL frontends lower into schema-owned requests before producing mutation plans, and field-path index builds, expression-index builds, secondary-index drops, full row rewrites, and unsupported mutation actions are explicit physical-work facts while publication remains fail-closed until rebuild execution exists.
...
- `0.151.5` removes the accepted-runtime generated expression candidate lane from `VisibleIndexes`, access planning, residual reranking, and explain access-choice projection, drops generated model parameters from accepted visible-index helpers, guards startup index rebuild so accepted `SchemaInfo` expression contracts are the only runtime planner/rebuild source for expression indexes, and hard-cuts accepted commit/cache schema fingerprints to hash the accepted persisted schema snapshot directly instead of appending generated `EntityModel` index metadata to runtime fingerprint identity.
...
- `0.151.2` routes runtime visible-index planning through accepted expression-index contracts when the accepted schema view carries them, adds owned accepted expression key items to reduced semantic index contracts, keeps generated expression candidates only as the explicit fallback lane, and extends access-choice, explain, covering, range, prefix, and index-key materialization helpers to understand accepted expression key items.
...
- `0.150.4` reduces runtime access capability index shape facts so route, aggregate, grouped-order, executable-summary, and logical slot-target preparation consume index name, uniqueness, key-item, and arity facts instead of reopening full generated `IndexModel` values from `IndexShapeDetails`, while keeping access-path DTO construction and expression-index key items on their explicit temporary bridge.
- `0.150.3` routes runtime field-path cursor-anchor and forward/unique write index key construction through accepted `SchemaInfo` index contracts, using accepted index ordinals, accepted row-layout slots, accepted store paths, and accepted uniqueness for field-path keys and preflight membership decoding, narrows cursor-anchor authority to the index-range contract instead of a standalone generated index argument, gates commit preflight field-path forward-index planning on accepted index contracts, splits runtime EXPLAIN finalization so accepted visible indexes no longer share an implicit generated-model fallback, and reduces lowered executor scan leaves to scan facts after raw bounds are materialized while keeping expression-index key construction on the explicit generated deferred lane.
- `0.150.2` adds a planner-facing accepted field-path index contract to runtime `VisibleIndexes`, carrying accepted index name, store, uniqueness, field paths, and row-layout slots alongside the temporary generated `IndexModel` bridge, and routes runtime order-only field-path access fallback through accepted index order terms while leaving the generated order fallback explicitly model-only and preserving the generated expression-index lane until accepted expression contracts land.
...
- `0.149.30` closes the generated persisted-row bridge cleanup by naming test-only generated serialized patch materializers, dense row emitters, raw-row readers, and row convenience helpers as generated/test surfaces, while moving local wasm staging and audit artifact labels from dfx-era paths to ICP CLI-era `.icp` paths.
...
- `0.149.23` routes scalar projection expression/plan compilation, prepared predicate compilation/scalar fast-path classification, and grouped aggregate execution target slots through accepted `SchemaInfo` instead of reopening generated model field metadata, extending accepted schema authority to scalar projection planning, predicate preparation, and grouped aggregate handoff.
- `0.149.22` routes scalar aggregate fast-count nullability decisions, scalar aggregate expression compilation, cursor index-anchor key slot resolution, authority-owned covering projection field slots, aggregate execution-explain route shapes, and session/SQL execution-explain route facts through request/authority-carried accepted `SchemaInfo` and `EntityAuthority` instead of reopening generated model field metadata, extending accepted schema authority to terminal fast-path eligibility, aggregate reducer preparation, cursor-anchor index key construction, covering read admission, aggregate descriptor routing, and descriptor diagnostics.
- `0.149.21` makes the shared persisted-row contract decode, scalar validation, non-scalar validation, and raw-row canonicalization helpers fail closed in production when they are handed a non-accepted row contract, leaves generated row-contract fallbacks compiled only for explicit tests, and routes runtime compiled SQL EXPLAIN command lowering through accepted `SchemaInfo` instead of generated SELECT schema fallback.
- `0.149.20` centralizes typed accepted `EntityAuthority` selection at the session boundary, routes typed shared query planning, SQL compiled-command cache identity, SQL SELECT/EXPLAIN execution dispatch, SQL EXPLAIN cache/route reads, prepared execution descriptors, and SQL INSERT SELECT/UPDATE selector reads through that accepted authority, removes the dead raw-authority schema/query-cache normalization helpers, and makes raw `EntityAuthority` bootstrap layout-free unless an accepted contract or explicit test-only generated layout is attached.
- `0.149.19` routes fluent grouped fields, fluent grouped HAVING fields, fluent aggregate/projection terminals, SQL grouped fields, SQL grouped HAVING literal canonicalization, and SQL aggregate field-target slot resolution through session/lowering-selected accepted `SchemaInfo`, sharing the accepted schema projection with SQL global aggregate execution and removing its dead generated `EntityAuthority` bootstrap lane instead of resolving those runtime slots from generated model order.
- `0.149.18` routes scalar continuation cursor boundary validation, prepared execution static-shape finalization, and scalar aggregate expression compilation through authority/session-carried accepted `SchemaInfo`, removing those generated schema lookups while keeping generated static-shape and scalar-expression wrappers test-only.
- `0.149.17` removes executor-authority generated schema reopening from runtime plan validation, requiring executor-bound plans to carry planner-frozen static shape finalized from accepted schema metadata while preserving schema-free runtime access invariants such as inverted key-range rejection.
- `0.149.16` derives the test-only save executor schema metadata from accepted snapshots instead of generated `E::MODEL` schema caches, finalizes the shared query cache trivial scalar fast path with session-selected accepted `SchemaInfo`, and routes session SQL/fluent explain binding plus access-choice projection through accepted schema metadata, keeping those executor metadata paths accepted-schema-backed.
- `0.149.15` adds a required accepted row-decode field lookup, routes accepted typed-save authorship, entity invariant, and strong-relation validation through it, carries accepted `SchemaInfo` and accepted commit fingerprints into save execution, removes the generated save-relation validator, and builds commit/delete/rebuild/diagnostic relation row contracts from accepted snapshots after schema acceptance so runtime validation no longer reopens generated schema metadata for those row-contract decisions.
- `0.149.14` makes accepted structural row field-name, leaf-codec, value-storage scalar fast-path, and primary-key validation lookups fail closed when an accepted contract is selected, preventing those row-reader authority paths from falling back to generated field metadata.
- `0.149.13` makes conditional forward-index predicates, accepted executor row layouts, structural field-path raw-byte access, accepted structural patch materialization, and global DISTINCT grouped aggregate runtime preparation resolve field slots through accepted row contracts instead of generated model field order during runtime planning and decode, makes the generated-only prepared-plan constructor test-only, and removes metadata-only generated authority construction from save validation and schema bootstrap.
...
- `0.147.18` splits the row-reader field surface so runtime value decode can request narrow decode facts while typed materialization keeps its generated-compatible field bridge.
...
- `0.145.12` trims projection and ranked-terminal vector growth by reserving final row, reorder, and winner buffers from known input sizes.
...
- `0.137.2` makes SQL aggregate queries cheaper by sharing repeated aggregate expressions per row and feeding aggregate reducers from the scalar runtime instead of building an intermediate aggregate row page.
...
- `0.126.3` finishes the `FieldValue` breakup, so typed keys, structured runtime conversion, and generated schema value wiring now go through `KeyValueCodec`, `ValueCodec`, and `ValueSurfaceMeta` instead of one catch-all runtime value trait.
...
- `0.124.2` closes the remaining expression-side structural cleanup around shared planner queries, ordering contracts, and covering-route preparation, so aggregate-input canonicalization, parser tree walks, order matching, route order proofs, and covering-read shaping now reuse shared owners instead of parallel helper ladders, including the last pure-vs-hybrid wrappers and repeated per-field access re-entry inside covering projection planning.
...
- `0.124.0` centralizes the first scalar-function ownership surfaces on shared enum owners, so planner typing, boolean predicate shaping, SQL parser/lowering dispatch, aggregate-input constant folding, and casefold wrapper handling now read shared planner-owned surfaces instead of repeating small local ladders, while `CEIL(...)` and `CEILING(...)` now share one canonical function identity.
...
- `0.109.1` tightens canonical semantic identity itself, so canonical scalar `filter_expr` is now the only shared plan-cache filter identity when it exists and unary semantic filters like `IS TRUE` and `IS NOT TRUE` no longer alias on the shared query-plan cache.
...
See detailed breakdown:
[docs/changelog/0.102.md](docs/changelog/0.102.md)
...
- `0.94.2` tightens the aggregate `FILTER` line around the original design by making grouped `HAVING`, grouped aggregate `FILTER`, and structural SQL aggregate `FILTER` all share the same TRUE-only boolean admission helper, and by fixing one grouped `COUNT(*) FILTER (...)` route bug so filtered grouped counts stay on the normal reducer path instead of falling onto the bare row-count fast path and ignoring the filter.
...
- `0.92.3` finishes the SQL-side cache simplification by deleting the last SQL prepared-plan cache layer, so repeated SQL `SELECT` calls now reuse only the compiled SQL command cache plus the shared lower query-plan cache, and the focused SQL tests plus the full PocketIC audit still show the intended repeat-query and warm-path behavior after that cut.
- `0.92.2` hardens the SQL cache line’s PocketIC proof instead of changing the runtime model again, by adding stronger repeat-query, warm-after-update, and named report-row contract tests so scalar, expression-order, grouped, guarded, `CASE`, and `DISTINCT` query shapes all prove that normal reuse stays on the intended visible cache path, and the full audit report now carries grouped and guarded cold/warm parity rows too.
- `0.92.1` keeps simplifying the SQL cache line by deleting more duplicate session-side `SELECT` preparation, grouped result-packaging, and projection-shaping paths, so parsed SQL, lowered SQL, grouped SQL, and diagnostics now flow through fewer internal wrappers while preserving the same cached query behavior, and it also adds stronger `CASE`/`HAVING`/`DISTINCT`/ordering cache-key tests so nearby query shapes do not accidentally reuse the same cache entry.
- `0.92.0` starts the SQL cache simplification line by making normal compiled `SELECT` reuse flow through one clearer compiled-query identity instead of an overlapping fallback path, while also finishing the last grouped SQL and ordering follow-through fixes around searched `CASE`, aggregate-alias `HAVING`, filtered aggregate alias ordering, expression-owned `ORDER BY`, and the typed fluent ordering hard cut, so the shipped cache baseline starts from the real unified query surface instead of a few lingering edge-case failures and transitional order APIs.
...
- `0.91.4` keeps the unified SQL-expression line clean by deleting leftover clause-specific validation and alias-normalization helpers, making lowering the one authority for shipped `WHERE` surface checks, trimming a large amount of duplicated SQL-lowering test scaffolding, following through with a broad internal aggregate and pipeline cleanup, and removing the last grouped-aggregate compatibility track so grouped planning, explain output, fingerprints, and cache keys now read one shared aggregate shape.
...
- `0.90.6` closes the next scalar ordering follow-through by carrying the canonical `ORDER BY <alias>` path through mixed text-plus-numeric alias ordering too, so queries that combine `TRIM(name) AS trimmed_name` with nested numeric aliases like `ROUND((strength + dexterity) / (level + 1), 2) AS normalized_power` now order correctly instead of failing with `unknown field`.
- `0.90.5` closes one more obvious scalar SQL combination gap by letting `ORDER BY <alias>` resolve over the already-supported unary text-function projection family, so shapes like `SELECT TRIM(name) AS trimmed_name ... ORDER BY trimmed_name` and `SELECT LENGTH(name) AS name_len ... ORDER BY name_len DESC` now work instead of failing at alias normalization.
- `0.90.4` closes another normal single-entity SQL combination gap by letting grouped queries keep the existing field-to-field `WHERE` predicate family, so grouped shapes like `SELECT class_name, COUNT(*) FROM character WHERE strength = dexterity GROUP BY class_name` and `SELECT level, AVG(strength) FROM character WHERE strength > dexterity GROUP BY level` now execute instead of failing closed at grouped planner validation, and grouped aggregate alias ordering still works on top of that residual predicate path.
- `0.90.3` finishes another obvious single-entity SQL combination gap by admitting chained and parenthesized arithmetic expressions on the existing scalar and aggregate-expression surface, so shapes like `strength + 1 * 2`, `ROUND((strength + dexterity) / 2, 2)`, and `ROUND(AVG(strength + 1 * 2), 2)` now work, and it also cleans up both singleton global aggregate `ORDER BY` and grouped aggregate alias ordering, including parenthesized wrapped aggregate inputs, so previously admitted aggregate-result queries no longer fail on those remaining combination edges.
...
```sql
SELECT LEFT(name, 2) AS short_name
FROM character
