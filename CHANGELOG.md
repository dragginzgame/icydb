...
- `0.72.3` keeps the same deterministic-planning line but makes the internals easier to trust: `EXPLAIN` now reuses the planner’s own chosen-index reasoning, ordered secondary reads now share one common `ORDER BY ..., primary_key` contract across pushdown and bounded-window routing, and the SQL read surfaces now reuse one shared lowered-`SELECT` structural binder without collapsing the separate computed-projection or aggregate lanes.
