...
- `0.152.7` adds schema-owned runner capability derivation, admission checks, runner preflight, and publication-boundary preflight checks for mutation execution plans, so future physical rebuild/cleanup runners can be checked against accepted field-path rebuild, expression-index rebuild, secondary-index cleanup, validation, invalidation, and rewrite capabilities before publication policy is widened.
