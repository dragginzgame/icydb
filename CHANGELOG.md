...
- `0.109.1` tightens the canonical identity and planner cleanup work around semantic reuse, so canonical scalar `filter_expr` is now the only shared plan-cache filter identity when it exists, unary semantic filters like `IS TRUE` and `IS NOT TRUE` no longer alias on the shared query-plan cache, the `AND` planner’s existing empty-child, singleton primary-key, required-order, and secondary-family route wins now flow through one explicit family comparison path instead of a stack of hidden early returns, and non-index `EXPLAIN EXECUTION` roots now reuse planner-owned non-index winner reasons already frozen on the plan, including explicit fluent key-access overrides, planner-owned primary-key lookup and range reasons, richer singleton-primary-key and required-order primary-key-range family wins, and distinct builder-owned reasons for `LIMIT 0` and canonical `FALSE` empty-result routes.
