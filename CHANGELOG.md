...
- `0.154.3` adds a startup index-store preflight for the supported field-path index-add runner, classifying existing physical index entries as target-index versus unrelated-index occupancy before the runner executes while keeping populated stores fail-closed until scoped multi-index publication is enabled.
