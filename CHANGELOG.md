...
- `0.92.3` finishes the SQL-side cache simplification by collapsing the separate SQL prepared-select cache into cached compiled `SELECT` artifacts, so normal SQL repeats now flow through one shallower SQL-owned prepared path while keeping the same repeat-query cache attribution and the same shared lower-cache fallback role proven by the full PocketIC audit.
- `0.92.2` hardens the SQL cache line’s PocketIC proof instead of changing the runtime model again, by adding stronger repeat-query, warm-after-update, and named report-row contract tests so scalar, expression-order, grouped, guarded, `CASE`, and `DISTINCT` query shapes all prove that normal reuse still stays on the SQL-owned caches instead of drifting down into the shared lower cache, and the full audit report now carries grouped and guarded cold/warm parity rows too.
