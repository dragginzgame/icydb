...
- `0.90.4` finishes one more grouped aggregate ordering combination gap by preserving arithmetic parentheses when grouped aggregate aliases are rewritten back into `ORDER BY` terms, so parenthesized shapes like `ROUND(AVG((strength + dexterity) / 2), 2) AS avg_balanced_power ... ORDER BY avg_balanced_power` now work instead of being flattened into the wrong expression and rejected.
- `0.90.3` finishes another obvious single-entity SQL combination gap by admitting chained and parenthesized arithmetic expressions on the existing scalar and aggregate-expression surface, so shapes like `strength + 1 * 2`, `ROUND((strength + dexterity) / 2, 2)`, and `ROUND(AVG(strength + 1 * 2), 2)` now work, and it also cleans up both singleton global aggregate `ORDER BY` and grouped wrapped-aggregate alias ordering so previously admitted aggregate-result queries no longer fail on those remaining combination edges.
