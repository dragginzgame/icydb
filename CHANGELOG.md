...
- `0.134.4` audits and cleans up `db/index` boundaries, then records the `db/predicate` ownership audit so index planning, predicate semantics, range lowering, and raw-entry reads have clearer next cleanup targets without changing query, commit, rebuild, or recovery behavior.
