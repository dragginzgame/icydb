...
- `0.61.3` keeps flattening the simple-entity runtime floor by removing the old per-entity `LoadExecutor<E>::execute` wrapper, pushing more SQL dispatch work onto shared structural helpers, and moving reverse-relation index preparation off a fully typed per-entity loop, which cuts the `one_simple -> ten_simple` shrunk slope from `729,703` to `672,019` bytes and the gzipped slope from `29,585` to `25,967`.
