...
- `0.74.8` keeps the latest SQL read-path cleanup line going by removing more avoidable cloning in covering-row and structural projection materialization, so constant-only covering reads and generic structural SQL projection now borrow more of their slot data before building final output rows.
