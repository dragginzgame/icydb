...
- `0.91.5` keeps shrinking the internal SQL and query engine by collapsing duplicate scalar and grouped load execution paths, deleting repeated normal-vs-attributed session wrappers, and leaving one shared runtime-preparation path behind the shipped grouped SQL and projection behavior.
