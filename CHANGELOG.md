...
- `0.69.3` is a narrow follow-up that fixes generated canister memory registration on the new `canic-memory` API by re-queuing the application memory range at codegen time and then flushing pending registration during generated bootstrap, so generated `db()` startup keeps working without reopening the broader `0.69` executor work.
...
- `0.66.5` keeps the shipped SQL and storage behavior unchanged but wires the new crosscutting performance harness into the quickstart SQL canister and PocketIC integration flow, so the repo now records repeatable instruction-footprint baselines and optimization reruns instead of only structural perf guesses.
