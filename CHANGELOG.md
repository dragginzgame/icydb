...
- `0.149.22` routes scalar aggregate fast-count nullability decisions, scalar aggregate expression compilation, scalar projection expression/plan compilation, prepared predicate compilation/scalar fast-path classification, grouped aggregate execution target slots, cursor index-anchor key slot resolution, authority-owned covering projection field slots, aggregate execution-explain route shapes, and session/SQL execution-explain route facts through request/authority-carried accepted `SchemaInfo` and `EntityAuthority` instead of reopening generated model field metadata, extending accepted schema authority to terminal fast-path eligibility, aggregate reducer preparation, scalar projection planning, predicate preparation, grouped aggregate handoff, cursor-anchor index key construction, covering read admission, aggregate descriptor routing, and descriptor diagnostics.
