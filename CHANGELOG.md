...
- `0.69.4` keeps the current covering-read routes intact, makes narrow row-backed SQL reads about `6%–10%` cheaper by validating every stored field without fully materializing untouched complex fields, and tightens the supporting executor plumbing for streamed index-entry reads plus SQL-only covering output while explicitly documenting the follow-up executor ideas that were measured and rejected.
