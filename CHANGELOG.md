...
- `0.150.6` removes full generated `IndexModel` retention from access-path DTO storage, lowered executor prefix/range specs after raw bounds are materialized, and executable access payload index facts, carrying reduced index name, ordinal, store path, key-item, arity, predicate, and uniqueness facts through cursor-anchor validation, covering scans, physical scan leaves, residual access-bound stripping, covering metadata, cache-key projection, access-choice diagnostics, diagnostics-facing access projection, and access-planned query shell index-shape detection instead.
