...
- `0.76.3` adds a first reduced SQL write surface by admitting narrow `INSERT` and primary-key-only `UPDATE` statements on the typed dispatch lane, adds PocketIC and unit coverage for those writes, and includes the local test and clippy cleanup needed to keep the `0.76` branch green for the next push.
