...
- `0.82.3` makes duplicate SQL global aggregate projections like repeated `COUNT(...)` columns reuse one compiled aggregate terminal instead of recomputing the same reduction for every repeated output slot, while keeping the original SQL column order and labels unchanged.
