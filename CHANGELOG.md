...
- `0.84.4` closes the line with developer-tooling cleanup: local bootstrap now ensures `python3` is present when running `install-dev` and `update-dev`, and the grouped perf harness/report helpers were tidied enough to keep the new attribution surfaces clippy-clean without changing runtime behavior.
