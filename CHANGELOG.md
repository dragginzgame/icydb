...
- `0.82.8` restores the typed public write API so single-row writes return the saved entity directly, batch writes return plain `Vec<_>`, and plain typed deletes return row counts instead of a SQL-style mutation wrapper, bringing the facade back in line with the core typed write contract.
