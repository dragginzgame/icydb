...
Runtime SQL query endpoints for canisters are owned by the canister/facade
boundary, not generated by `icydb-model`.

### Ownership Rule

* `icydb-model` owns generated store/session wiring, not public canister SQL routing glue.
* If a canister exposes `query(sql)` or similar SQL endpoints, those functions are authored in the canister crate.
...
* Canister-owned routing helpers may compose SQL-owned helpers, but must not re-implement SQL identifier semantics.
