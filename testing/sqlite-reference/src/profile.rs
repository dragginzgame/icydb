...
const REQUIRED_SQLITE_REFERENCE_SCENARIOS: &[SqliteReferenceScenario] = &[
    SqliteReferenceScenario {
        ...
        sql_template: "SELECT u.name AS display_name, LENGTH(u.name) AS name_len FROM {entity} AS u WHERE u.age >= 24 AND u.age <= 43 ORDER BY display_name ASC, u.age ASC LIMIT 2 OFFSET 1",
    ...
    SqliteReferenceScenario {
        ...
        sql_template: "SELECT age, COUNT(*) AS row_count FROM {entity} WHERE age = age GROUP BY age HAVING COUNT(*) >= 1 ORDER BY row_count DESC, age ASC LIMIT 10",
        columns: TWO_INTEGER_COLUMNS,
        row_order: SqliteReferenceRowOrder::Ordered,
