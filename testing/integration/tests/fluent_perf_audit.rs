...
struct FluentPerfScenarioSample {
    ...
    baseline_avg_grouped_stream_local_instructions: Option<u64>,
    baseline_avg_grouped_fold_local_instructions: Option<u64>,
    baseline_avg_grouped_finalize_local_instructions: Option<u64>,
    baseline_avg_grouped_count_borrowed_hash_computations: Option<u64>,
...
fn maybe_write_blessed_baseline(samples: &[FluentPerfScenarioSample]) {
    ...
    let rows = samples
        ...
        .map(|sample| FluentPerfBaselineRow {
            ...
            avg_grouped_stream_local_instructions: sample.avg_grouped_stream_local_instructions,
            avg_grouped_fold_local_instructions: sample.avg_grouped_fold_local_instructions,
            avg_grouped_finalize_local_instructions: sample.avg_grouped_finalize_local_instructions,
            avg_grouped_count_borrowed_hash_computations: sample
                .avg_grouped_count_borrowed_hash_computations,
            avg_grouped_count_bucket_candidate_checks: sample
...
fn sample_perf_scenario(
    ...
    FluentPerfScenarioSample {
        ...
        baseline_avg_grouped_stream_local_instructions: baseline_row
            .map(|row| row.avg_grouped_stream_local_instructions),
        baseline_avg_grouped_fold_local_instructions: baseline_row
            .map(|row| row.avg_grouped_fold_local_instructions),
        baseline_avg_grouped_finalize_local_instructions: baseline_row
            .map(|row| row.avg_grouped_finalize_local_instructions),
        baseline_avg_grouped_count_borrowed_hash_computations: baseline_row
            .map(|row| row.avg_grouped_count_borrowed_hash_computations),
        baseline_avg_grouped_count_bucket_candidate_checks: baseline_row
            .map(|row| row.avg_grouped_count_bucket_candidate_checks),
        baseline_avg_grouped_count_existing_group_hits: baseline_row
...
fn user_fluent_scenarios() -> Vec<FluentPerfScenario> {
    vec![
        same_key_scenario(
            ...
            "ordered_scalar",
            r#"db()?.load::<PerfAuditUser>().order_term(asc("id")).partial_window(2)"#,
        ),
        same_key_scenario(
            "user.age.order_only.asc.limit3",
            FluentPerfSurface::User,
            "ordered_scalar",
            r#"db()?.load::<PerfAuditUser>().order_term(asc("age")).order_term(asc("id")).partial_window(3)"#,
        ),
        parity_scenario(
            ...
            FluentPerfSurface::User,
            "parity_cold_query",
            r#"db()?.load::<PerfAuditUser>().order_term(asc("age")).order_term(asc("id")).partial_window(2)"#,
        ...
        same_key_scenario(
            ...
            FluentPerfSurface::User,
            "predicate_scalar",
            r#"db()?.load::<PerfAuditUser>().filter_eq("active", true).order_term(asc("age")).order_term(asc("id")).partial_window(3)"#,
        ),
        same_key_scenario(
            "user.field_compare.age_eq_age_nat.limit3",
            FluentPerfSurface::User,
            "field_compare",
            r#"db()?.load::<PerfAuditUser>().filter_eq_field("age", "age_nat").order_term(asc("age")).order_term(asc("id")).partial_window(3)"#,
        ),
        same_key_scenario(
            "user.field_between.rank_age_age.limit3",
            FluentPerfSurface::User,
            "field_between",
            r#"db()?.load::<PerfAuditUser>().filter_between_fields("rank", "age", "age").order_term(asc("age")).order_term(asc("id")).partial_window(3)"#,
        ),
        same_key_scenario(
            "user.rank.in_list.limit3",
            FluentPerfSurface::User,
            "membership",
            r#"db()?.load::<PerfAuditUser>().filter_in("rank", [17, 28, 30]).order_term(asc("age")).order_term(asc("id")).partial_window(3)"#,
        ),
        same_key_scenario(
            "user.grouped.age_count.limit10",
            FluentPerfSurface::User,
            "grouped",
            r#"db()?.load::<PerfAuditUser>().group_by("age")?.aggregate(count()).order_term(asc("age")).partial_window(10)"#,
        ),
        repeat_scenario(
...
fn account_fluent_scenarios() -> Vec<FluentPerfScenario> {
    vec![
        same_key_scenario(
            ...
            "predicate_scalar",
            r#"db()?.load::<PerfAuditAccount>().filter_eq("active", true).order_term(asc("handle")).order_term(asc("id")).partial_window(3)"#,
        ),
        same_key_scenario(
            "account.gold_active.order_handle.asc.limit3",
            FluentPerfSurface::Account,
            "compound_predicate",
            r#"db()?.load::<PerfAuditAccount>().filter_eq("active", true).filter_eq("tier", "gold").order_term(asc("handle")).order_term(asc("id")).partial_window(3)"#,
        ),
        same_key_scenario(
            "account.score_gte_75.order_score.limit3",
            FluentPerfSurface::Account,
            "range_scalar",
            r#"db()?.load::<PerfAuditAccount>().filter_gte("score", 75).order_term(asc("score")).order_term(asc("id")).partial_window(3)"#,
        ),
        repeat_scenario(
...
fn token_fluent_scenarios() -> Vec<FluentPerfScenario> {
    vec![
        same_key_scenario(
            ...
            r#"db()?.load::<PerfAuditToken>().filter_eq("collection_id", target).filter_in("stage", ["Draft", "Review"]).order_term(asc("id")).partial_window(50)"#,
        ),
        parity_scenario(
        ...
        same_key_scenario(
            ...
            FluentPerfSurface::Token,
            "branch_set_full_entity",
            r#"db()?.load::<PerfAuditToken>().filter_eq("collection_id", target).filter_in("stage", ["Draft", "Draft", "Review"]).order_term(asc("id")).partial_window(50)"#,
        ),
        same_key_scenario(
...
const fn warm_token_branch_set_page_scenario() -> FluentPerfScenario {
    FluentPerfScenario {
        ...
        query_label: r#"db()?.load::<PerfAuditToken>().filter_eq("collection_id", target).filter_in("stage", ["Draft", "Review"]).order_term(asc("id")).partial_window(50)"#,
        sample_count: 1,
        query_loop_count: 1,
