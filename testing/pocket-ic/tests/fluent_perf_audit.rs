...
fn user_fluent_scenarios() -> Vec<FluentPerfScenario> {
    vec![
        ...
        same_key_scenario(
            ...
            FluentPerfSurface::User,
            "predicate_scalar",
            r#"db().load::<PerfAuditUser>().filter(FieldRef::new("active").eq(true)).order_term(asc("age")).order_term(asc("id")).limit(3)"#,
        ),
        same_key_scenario(
            "user.field_compare.age_eq_age_nat.limit3",
            FluentPerfSurface::User,
            "field_compare",
            r#"db().load::<PerfAuditUser>().filter(FieldRef::new("age").eq_field("age_nat")).order_term(asc("age")).order_term(asc("id")).limit(3)"#,
        ),
        same_key_scenario(
            "user.field_between.rank_age_age.limit3",
            FluentPerfSurface::User,
            "field_between",
            r#"db().load::<PerfAuditUser>().filter(FieldRef::new("rank").between_fields("age", "age")).order_term(asc("age")).order_term(asc("id")).limit(3)"#,
        ),
        same_key_scenario(
            "user.rank.in_list.limit3",
            FluentPerfSurface::User,
            "membership",
            r#"db().load::<PerfAuditUser>().filter(FieldRef::new("rank").in_list([17, 28, 30])).order_term(asc("age")).order_term(asc("id")).limit(3)"#,
        ),
        same_key_scenario(
...
fn account_fluent_scenarios() -> Vec<FluentPerfScenario> {
    vec![
        same_key_scenario(
            ...
            "predicate_scalar",
            r#"db().load::<PerfAuditAccount>().filter(FieldRef::new("active").eq(true)).order_term(asc("handle")).order_term(asc("id")).limit(3)"#,
        ),
        same_key_scenario(
        ...
        same_key_scenario(
            ...
            FluentPerfSurface::Account,
            "range_scalar",
            r#"db().load::<PerfAuditAccount>().filter(FieldRef::new("score").gte(75)).order_term(asc("score")).order_term(asc("id")).limit(3)"#,
        ),
        repeat_scenario(
