...
impl FinalizedQueryDiagnostics {
    ...
    pub(in crate::db) fn render_json_canonical(&self) -> Result<String, QueryError> {
        let mut out = String::new();
        write_execution_json(self, &mut out).map_err(|_| QueryError::invariant())?;
        Ok(out)
    }
}
...
fn write_execution_json(
    ...
    object.field_with("admission", |out| match diagnostics.admission() {
    ...
    object.field_with("execution", |out| {
        ...
        write_execution_node_json(diagnostics.execution(), &mut node_id_counter, out)?;
        Ok(())
    })?;
    object.field_str_slice("route_diagnostics", &diagnostics.route_diagnostics)?;
    object.field_str_slice("logical_diagnostics", &diagnostics.logical_diagnostics)?;
    object.field_with("reuse", |out| {
        let Some(reuse) = diagnostics.reuse else {
            out.write_str("null")?;
            return Ok(());
        };
...
fn write_admission_json(admission: &QueryAdmissionSummary, out: &mut dyn Write) -> fmt::Result {
    let mut object = JsonWriter::begin_object(out)?;
    ...
    object.field_with("materialization", |out| {
        ...
        materialization_object
            .field_bool("materialized_sort", materialization.materialized_sort())?;
        write_optional_u32(
...
fn write_optional_u32(object: &mut JsonWriter<'_>, key: &str, value: Option<u32>) -> fmt::Result {
    match value {
...
fn write_optional_u64(object: &mut JsonWriter<'_>, key: &str, value: Option<u64>) -> fmt::Result {
    match value {
