...
fn eval_text_list_membership_compare(
    ...
    eval_list_membership_compare_result(op, text_in_list(actual, value, mode))
}
...
fn compare_scalar_text(actual: &str, expected: &str, mode: TextMode) -> Ordering {
    match mode {
        ...
        TextMode::Ci => casefold_text(actual).cmp(&casefold_text(expected)),
    }
}
...
pub(in crate::db::predicate::runtime) fn text_contains_scalar(
    ...
    match mode {
        ...
        TextMode::Ci => casefold_text(actual).contains(&casefold_text(needle)),
    }
}
...
fn text_starts_with_scalar(actual: &str, prefix: &str, mode: TextMode) -> bool {
    match mode {
        ...
        TextMode::Ci => casefold_text(actual).starts_with(&casefold_text(prefix)),
    }
}
...
fn text_ends_with_scalar(actual: &str, suffix: &str, mode: TextMode) -> bool {
    match mode {
        ...
        TextMode::Ci => casefold_text(actual).ends_with(&casefold_text(suffix)),
    }
}
