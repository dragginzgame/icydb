...
pub(in crate::db) enum SqlDdlBindError {
    ...
    #[error(
        "index '{index_name}' is generated by the entity model and cannot be dropped with SQL DDL"
    )]
    GeneratedIndexDropRejected { index_name: String },

    #[error("index '{index_name}' is not a supported DDL-droppable field-path index")]
