...
impl GroupedCountState {
    ...
    fn increment_borrowed_group_probe(
        ...
        let lookup = lookup_existing_group(self.groups.as_slice(), &self.bucket_index);
        self.complete_group_lookup(grouped_execution_context, lookup?, materialize_new_group)
    }
    ...
    fn complete_group_lookup(
        ...
        if let Some(existing_index) = existing_index {
            self.increment_existing_group(existing_index)?;
    ...
    fn increment_owned_group_key(
        ...
        let existing_index = self.bucket_index.get(&group_hash).and_then(|bucket| {
            bucket.as_slice().iter().copied().find(|existing_index| {
                self.groups
                    .get(*existing_index)
