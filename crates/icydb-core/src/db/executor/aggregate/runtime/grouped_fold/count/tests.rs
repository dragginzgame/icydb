...
fn grouped_count_bounded_candidate_selection_keeps_smallest_canonical_window() {
    let rows = vec![
        (
            crate::db::executor::group::GroupKey::from_group_values(vec![Value::Nat64(9)])
                .expect("group key"),
            9,
        ),
        (
            crate::db::executor::group::GroupKey::from_group_values(vec![Value::Nat64(2)])
                .expect("group key"),
            2,
        ),
        (
            crate::db::executor::group::GroupKey::from_group_values(vec![Value::Nat64(5)])
                .expect("group key"),
            5,
        ),
        (
            crate::db::executor::group::GroupKey::from_group_values(vec![Value::Nat64(1)])
                .expect("group key"),
            1,
        ),
    ];
    ...
    assert_eq!(
        selected
            ...
            .collect::<Vec<_>>(),
        vec![
            (Value::List(vec![Value::Nat64(1)]), 1),
            (Value::List(vec![Value::Nat64(2)]), 2),
            (Value::List(vec![Value::Nat64(5)]), 5),
        ],
