...
fn grouped_count_field_skips_null_slot_values() {
    ...
    let non_null_row = RowView::new(vec![Some(Value::Nat64(7))]);
...
fn grouped_sum_field_skips_null_slot_values_and_accumulates_numeric_rows() {
    ...
    let group = text_group_key("alpha");
    let numeric_row = RowView::new(vec![Some(Value::Nat64(7))]);
    let second_numeric_row = RowView::new(vec![Some(Value::Nat64(9))]);
    let null_row = RowView::new(vec![Some(Value::Null)]);
...
fn grouped_avg_field_skips_null_slot_values_and_accumulates_numeric_rows() {
    ...
    let group = text_group_key("alpha");
    let numeric_row = RowView::new(vec![Some(Value::Nat64(6))]);
    let second_numeric_row = RowView::new(vec![Some(Value::Nat64(12))]);
    let null_row = RowView::new(vec![Some(Value::Null)]);
...
fn grouped_id_terminals_finalize_to_structural_primary_key_values() {
    assert_eq!(
        ...
        vec![(Value::Text("alpha".to_string()), Value::Nat64(3))],
    ...
    assert_eq!(
        ...
        vec![(Value::Text("alpha".to_string()), Value::Nat64(7))],
