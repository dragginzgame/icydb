...
use crate::{
    db::{
        ...
        executor::{
            ...
            CoveringComponentValues, CoveringProjectionComponentRows,
            CoveringProjectionComponentWindow, ExecutableAccess, PreparedExecutionPlan,
            ...
            fold_covering_projection_component_rows_in_window, page_window_state,
    ...
    E: EntityKind + EntityValue,
{
    ...
    fn derive_bytes_terminal_fast_path_contract_from_prepared(
        ...
        let direction = match prepared.order_spec() {
            ...
            Some(order) => {
                ...
                match order.primary_key_only_direction_fields(&primary_key_names) {
        ...
        let shape_facts = executable.shape_facts().single_path_facts()?;

        primary_key_stream_window_shape_supported(&shape_facts)
            .then_some(BytesTerminalFastPathContract::PrimaryKeyWindow(direction))
            .or_else(|| {
                ordered_key_stream_window_shape_supported(&shape_facts).then_some(
    ...
    fn bytes_by_covering_index_if_eligible(
        ...
        let primary_key_names = prepared.logical_plan().primary_key_names()?;
    ...
    fn bytes_by_index_order_covering_pairs(
        ...
        fold_covering_projection_component_rows_in_window(
            ...
            prepared.consistency(),
            covering_requires_row_presence_check(),
            CoveringProjectionComponentWindow::new(offset, limit),
            0_u64,
            |total, _data_key, components| {
                let Some(value_len) = bytes_by_single_covering_component_len(components)? else {
                    return Ok(None);
                };

                Ok(Some(saturating_add_payload_len(total, value_len)))
            },
        )
    }
