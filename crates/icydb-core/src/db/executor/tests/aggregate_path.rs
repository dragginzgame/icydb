...
fn serialized_field_payload_bytes_for_pushdown_rows(
    ...
    response.iter().fold(0u64, |acc, row| {
        let value = match field {
            "group" => Value::Nat64(u64::from(row.entity_ref().group)),
            "rank" => Value::Nat64(u64::from(row.entity_ref().rank)),
    ...
    E: EntityKind,
{
    ...
    match resolved_index.and_then(|index| E::MODEL.fields.get(index)) {
        Some(model_field) => crate::db::query::plan::FieldSlot::from_test_accepted_kind(
            index,
            field,
            crate::db::schema::AcceptedFieldKind::from_model_kind(model_field.kind),
        ),
        None => crate::db::query::plan::FieldSlot::from_test_slot(index, field),
    }
}
...
fn u32_eq_predicate(field: &str, value: u32) -> Predicate {
    strict_compare_predicate(field, CompareOp::Eq, Value::Nat64(u64::from(value)))
}
...
fn aggregate_path_exists_index_range_window_scans_offset_plus_one() {
    ...
    let mut logical_plan = AccessPlannedQuery::new(
        AccessPath::index_range(
            ...
            vec![],
            Bound::Included(Value::Nat64(101)),
            Bound::Excluded(Value::Nat64(106)),
        ),
        MissingRowPolicy::Ignore,
    );
    logical_plan.scalar_plan_mut().order = Some(OrderSpec {
