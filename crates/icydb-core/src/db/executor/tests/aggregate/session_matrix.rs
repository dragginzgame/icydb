...
fn session_load_explain_execution_text_and_json_snapshot_for_strict_index_prefix_shape() {
    ...
    let expected_text = r#"IndexPrefixScan execution_mode=Materialized node_id=0 layer=scan execution_mode_detail=materialized predicate_pushdown_mode=none fast_path_selected=true fast_path_reason=secondary_order_pushdown_eligible access=IndexPrefix(group_rank) covering_scan=false node_properties=access_choice_alternatives=List([]),access_choice_chosen=Text("index:group_rank"),access_choice_chosen_reason=Text("single_candidate"),access_choice_rejections=List([]),continuation_mode=Text("initial"),covering_scan_reason=Text("order_requires_materialization"),fast_path_rejections=List([Text("primary_key=pk_order_fast_path_ineligible"), Text("index_range=index_range_limit_pushdown_disabled")]),fast_path_selected=Text("secondary_prefix"),fast_path_selected_reason=Text("secondary_order_pushdown_eligible"),prefix_len=Uint(1),projected_fields=List([Text("id"), Text("group"), Text("rank"), Text("label")]),projection_pushdown=Bool(false),resume_from=Text("none"),scan_direction=Text("asc")
  IndexPredicatePrefilter execution_mode=Materialized node_id=1 layer=pipeline execution_mode_detail=materialized predicate_pushdown_mode=full predicate_pushdown=strict_all_or_none node_properties=pushdown=Text("group=Uint(7)")
  SecondaryOrderPushdown execution_mode=Materialized node_id=2 layer=pipeline execution_mode_detail=materialized predicate_pushdown_mode=none node_properties=index=Text("group_rank"),prefix_len=Uint(1)
  OrderByMaterializedSort execution_mode=Materialized node_id=3 layer=pipeline execution_mode_detail=materialized predicate_pushdown_mode=none node_properties=order_satisfied_by_index=Bool(false)
  LimitOffset execution_mode=Materialized node_id=4 layer=terminal execution_mode_detail=materialized predicate_pushdown_mode=none limit=2 cursor=false node_properties=offset=Uint(1)"#;
    assert_eq!(
