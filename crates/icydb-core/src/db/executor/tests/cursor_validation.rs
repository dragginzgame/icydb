...
use crate::{
    db::{
        cursor::{
            ...
            CursorPayloadErrorCode, CursorPlanError, GroupedContinuationToken,
        },
        direction::Direction,
...
fn load_cursor_rejects_boundary_arity_mismatch_at_plan_time() {
    ...
    let boundary = CursorBoundary {
        ...
        slots: vec![CursorBoundarySlot::Present(Value::Nat64(10))],
    };
    let cursor =
        ContinuationToken::new_with_direction(signature, boundary, Direction::Asc, initial_offset)
            .encode()
...
fn grouped_cursor_rejects_descending_direction_at_plan_time() {
    ...
    std::assert_matches!(
        ...
        CursorPlanError::InvalidContinuationCursorPayload {
            reason: CursorPayloadErrorCode::GROUPED_DIRECTION_MISMATCH,
            index: None
        }
    );
}
