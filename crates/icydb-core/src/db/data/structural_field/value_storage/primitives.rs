...
// === Scalar Decode Helpers (parsed-head variants) ===

// Decode one parsed binary bytes payload without re-parsing the already owned
// structural head.
pub(super) fn decode_binary_required_bytes_from_parsed<'a>(
    raw_bytes: &'a [u8],
    tag: u8,
    len: u32,
    payload_start: usize,
...
// Decode one parsed binary text payload without re-parsing the already owned
// structural head.
pub(super) fn decode_binary_required_text_from_parsed<'a>(
    raw_bytes: &'a [u8],
    tag: u8,
    len: u32,
    payload_start: usize,
...
// Decode one parsed binary i64 payload without re-parsing the already owned
// structural head.
pub(super) fn decode_binary_required_i64_from_parsed(
    raw_bytes: &[u8],
    tag: u8,
    len: u32,
    payload_start: usize,
    ...
    let payload = parsed_scalar_payload_bytes(
        raw_bytes,
        len,
        payload_start,
    ...
    decode_i64_payload_bytes(payload, "i64")
}

// Decode one parsed binary u64 payload without re-parsing the already owned
// structural head.
pub(super) fn decode_binary_required_u64_from_parsed(
    raw_bytes: &[u8],
    tag: u8,
    len: u32,
    payload_start: usize,
    ...
    decode_u64_payload_bytes(payload, "u64")
}

// Return the parsed scalar payload after proving it consumes the full caller
// slice. This mirrors the scalar branch of `skip_binary_value` without
// re-parsing the root head.
fn parsed_scalar_payload_bytes<'a>(
    raw_bytes: &'a [u8],
    len: u32,
    payload_start: usize,
    ...
    raw_bytes
        .get(payload_start..expected_end)
