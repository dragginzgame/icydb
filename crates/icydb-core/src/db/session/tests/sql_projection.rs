...
fn execute_sql_projection_selects_record_subfields() {
    ...
    assert_eq!(
        rows,
        vec![
            vec![Value::Int64(7), Value::Text("ace".to_string())],
            vec![Value::Int64(3), Value::Text("bee".to_string())],
            vec![Value::Int64(11), Value::Text("cee".to_string())],
        ],
    ...
    assert_projection_row_case_matrix::<SessionSqlEntity>(
        ...
        &[
            (
                ...
                vec![
                    vec![Value::Text("bravo".to_string()), Value::Nat64(20)],
                    vec![Value::Text("alpha".to_string()), Value::Nat64(30)],
                    vec![Value::Text("charlie".to_string()), Value::Nat64(40)],
                ],
...
fn execute_sql_projection_computed_function_matrix_runs_from_session_boundary() {
    ...
    for (sql, expected_columns, expected_rows, context) in [
        ...
        (
            ...
            vec![
                vec![Value::Text("Ada".to_string()), Value::Nat64(33)],
                vec![Value::Text("Bob".to_string()), Value::Null],
            ],
...
fn execute_sql_projection_distinct_matrix_matches_expected_rows() {
    for (seed_rows, sql, expected_rows, expect_pk_rows, context) in [
        ...
        (
            ...
            vec![vec![Value::Nat64(25)], vec![Value::Nat64(30)]],
            false,
        ...
        if expect_pk_rows {
            ...
            assert_eq!(
                response
                    ...
                    .map(|row| row[1].clone())
                    .collect::<Vec<_>>(),
                vec![Value::Nat64(25), Value::Nat64(25)],
...
fn execute_sql_projection_distinct_limit_and_offset_use_deduped_ordered_stream() {
    for (sql, expected_rows, context) in [
        (
            ...
            vec![vec![Value::Nat64(10)], vec![Value::Nat64(20)]],
        ...
        (
            ...
            vec![vec![Value::Nat64(20)], vec![Value::Nat64(30)]],
...
fn execute_sql_projection_matrix_queries_match_expected_projected_rows() {
    ...
    let cases = vec![
        (
            ...
            vec![
                vec![Value::Text("matrix-c".to_string()), Value::Nat64(30)],
                vec![Value::Text("matrix-b".to_string()), Value::Nat64(20)],
            ],
        ),
        (
            "SELECT age \
