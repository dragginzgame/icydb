    ...
    let cases = [
        (
            ...
            "SELECT name, COUNT(*) \
             ...
             ORDER BY name ASC LIMIT 10",
            vec![
                (Value::Text("alpha".to_string()), vec![Value::Nat64(2)]),
                (Value::Text("bravo".to_string()), vec![Value::Nat64(1)]),
                (Value::Text("charlie".to_string()), vec![Value::Nat64(2)]),
            ],
        ),
        (
            "COUNT(age)",
        ...
        (
            ...
            "SELECT name, MIN(age) \
             ...
             ORDER BY name ASC LIMIT 10",
            vec![
                (Value::Text("alpha".to_string()), vec![Value::Nat64(10)]),
                (Value::Text("bravo".to_string()), vec![Value::Nat64(30)]),
                (Value::Text("charlie".to_string()), vec![Value::Nat64(40)]),
            ],
        ),
        (
            "MIN(DISTINCT age)",
        ...
        (
            ...
            "SELECT name, MAX(age) \
             ...
             ORDER BY name ASC LIMIT 10",
            vec![
                (Value::Text("alpha".to_string()), vec![Value::Nat64(20)]),
                (Value::Text("bravo".to_string()), vec![Value::Nat64(30)]),
                (Value::Text("charlie".to_string()), vec![Value::Nat64(50)]),
            ],
        ),
        (
            "COUNT(*) + SUM(age)",
    ...
    let cases = [
        (
            ...
            "SELECT name, COUNT(*) \
             ...
             GROUP BY name \
             ORDER BY name ASC LIMIT 10",
            vec![(Value::Text("alpha".to_string()), vec![Value::Nat64(2)])],
        ),
        (
            "filtered SUM(age)",
            "SELECT name, SUM(age) \
    ...
    let cases = [
        (
            ...
            "SELECT age, COUNT(*) \
             ...
             ORDER BY age ASC LIMIT 10",
            vec![
                (Value::Nat64(10), Value::Nat64(2)),
                (Value::Nat64(20), Value::Nat64(1)),
                (Value::Nat64(30), Value::Nat64(3)),
            ],
        ),
        (
...
fn grouped_select_helper_filter_aggregate_matrix_matches_expected_grouped_rows() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![
            (Value::Nat64(10), vec![Value::Nat64(0), Value::Null]),
            (
                Value::Nat64(20),
                vec![
                    Value::Nat64(1),
...
fn grouped_select_helper_filtered_aggregate_on_non_group_field_supports_alias_having_and_order() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![
            (Value::Text("gamma".to_string()), vec![Value::Nat64(3)]),
            (Value::Text("alpha".to_string()), vec![Value::Nat64(2)]),
        ],
...
fn grouped_select_helper_filtered_count_rows_on_non_group_field_uses_generic_reducers() {
    ...
    assert_eq!(
        ...
        vec![
            (Value::Text("alpha".to_string()), vec![Value::Nat64(2)]),
            (Value::Text("beta".to_string()), vec![Value::Nat64(1)]),
            (Value::Text("gamma".to_string()), vec![Value::Nat64(2)]),
        ],
...
fn grouped_select_helper_filtered_aggregate_order_alias_supports_null_test_boolean_compositions() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![
            (Value::Text("bravo".to_string()), vec![Value::Nat64(2)]),
            (Value::Text("alpha".to_string()), vec![Value::Nat64(1)]),
            (Value::Text("charlie".to_string()), vec![Value::Nat64(0)]),
        ],
...
fn grouped_select_helper_executes_field_to_field_predicate() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![(Value::Nat64(10), vec![Value::Nat64(2)])],
...
fn grouped_select_helper_executes_searched_case_where_expression() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![
            (Value::Nat64(20), vec![Value::Nat64(1)]),
            (Value::Nat64(30), vec![Value::Nat64(1)]),
            (Value::Nat64(40), vec![Value::Nat64(1)]),
        ],
...
fn grouped_select_helper_executes_coalesce_and_nullif_where_expression() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![(Value::Nat64(20), vec![Value::Nat64(1)])],
...
fn grouped_select_helper_executes_text_transform_where_expression() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![(Value::Nat64(10), vec![Value::Nat64(1)])],
...
fn grouped_select_helper_count_matrix_returns_expected_grouped_rows() {
    ...
    let cases = [
        (
            ...
            "SELECT age, COUNT(*) FROM SessionSqlEntity GROUP BY age ORDER BY age ASC LIMIT 10",
            vec![
                (Value::Nat64(20), Value::Nat64(2)),
                (Value::Nat64(32), Value::Nat64(1)),
            ],
        ),
        (
...
fn grouped_select_helper_executes_bounded_wrapped_aggregate_order_top_k_alias_rows() {
    ...
    assert_eq!(
        ...
        vec![
            (Value::Text("charlie".to_string()), vec![Value::Int64(99)]),
            (
                Value::Text("delta".to_string()),
                vec![Value::Decimal(crate::types::Decimal::from(50_u64))],
            ),
...
fn execute_sql_statement_grouped_payload_matrix() {
    ...
    let expected_rows = vec![
        ...
        (Value::Nat64(32), vec![Value::Nat64(1)]),
    ];
...
fn grouped_select_pagination_preserves_cursor_with_extra_group_projection_columns() {
    ...
    assert_eq!(
        runtime_output(first_page.rows()[0].group_key()[1].clone()).cmp_numeric(&Value::Nat64(11)),
    ...
    assert_eq!(
        runtime_output(second_page.rows()[0].group_key()[1].clone()).cmp_numeric(&Value::Nat64(21)),
        Some(std::cmp::Ordering::Equal),
    );
    assert_eq!(
        runtime_outputs(second_page.rows()[0].aggregate_values()),
...
fn grouped_select_allows_searched_case_projection_and_having() {
    ...
    assert_eq!(
        ...
        vec![
            (Value::Nat64(10), vec![Value::Text("multi".to_string())],),
            (Value::Nat64(20), vec![Value::Text("single".to_string())],),
        ],
...
fn grouped_select_allows_post_aggregate_having_expressions() {
    ...
    assert_eq!(
        grouped_result_rows(&execution),
        vec![
            (Value::Nat64(10), vec![Value::Nat64(2)]),
            (Value::Nat64(20), vec![Value::Nat64(1)]),
        ],
...
fn grouped_select_allows_unary_text_having_expressions() {
    ...
    assert_eq!(
        ...
        vec![
            (Value::Text("bravo".to_string()), vec![Value::Nat64(1)]),
            (Value::Text("charlie".to_string()), vec![Value::Nat64(1)]),
        ],
