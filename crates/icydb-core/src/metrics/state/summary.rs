//! Module: metrics::state::summary
//! Responsibility: rich per-entity metrics summary DTO and projection.
//! Does not own: mutable metrics state updates or top-level report windows.
//! Boundary: keeps per-entity rich report payload shape separate from raw state.

use candid::CandidType;
use serde::Deserialize;

use super::{EntityCounters, MetricRatio, ratio};
...
impl EntitySummary {
    ...
    pub(in crate::metrics) const fn activity_score(&self) -> u64 {
        self.load_calls
...
#[expect(clippy::too_many_lines)]
pub(in crate::metrics) fn entity_summary_from_counters(
    path: &str,
    ops: &EntityCounters,
) -> EntitySummary {
    EntitySummary {
