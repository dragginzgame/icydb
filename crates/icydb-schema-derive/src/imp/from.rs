...
impl Imp<List> for FromTrait {
    fn strategy(node: &List) -> Option<TraitStrategy> {
        ...
        Some(vec_collection_from_strategy(
            node.def(),
            item,
            quote!(::icydb::__macro::field_value_from_vec_into::<#item, I>(entries)),
            None,
        ))
    }
}
...
impl Imp<Set> for FromTrait {
    fn strategy(node: &Set) -> Option<TraitStrategy> {
        ...
        Some(vec_collection_from_strategy(
            node.def(),
            item,
            quote!(::icydb::__macro::field_value_from_vec_into_btree_set::<#item, I>(entries)),
    ...
    let tokens = Implementor::new(def, TraitKind::From)
        .set_tokens(quote! {
            fn from(entries: Vec<(IK, IV)>) -> Self {
                Self(::icydb::__macro::field_value_from_vec_into_btree_map::<#key, #value, IK, IV>(
                    entries,
                ))
            }
        })
...
fn newtype_from_strategy(def: &Def, item: &TokenStream) -> TraitStrategy {
    let tokens = Implementor::new(def, TraitKind::From)
        .set_tokens(quote! {
            fn from(t: T) -> Self {
                Self(::icydb::__macro::field_value_into::<#item, T>(t))
            }
        })
