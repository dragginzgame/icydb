...
pub(crate) use icydb_model_legacy::node::{APP_MEMORY_ID_MAX, APP_MEMORY_ID_MIN};
...
pub(crate) fn memory_id_out_of_range_error(
    ...
    (!icydb_model_legacy::node::memory_id_is_in_range(memory_id, min, max))
...
pub(crate) fn memory_id_reserved_error(label: &str, memory_id: u8) -> Option<String> {
    icydb_model_legacy::node::memory_id_is_reserved(memory_id)
...
pub(crate) fn app_memory_id_error(label: &str, memory_id: u8) -> Option<String> {
    (!icydb_model_legacy::node::app_memory_id_is_valid(memory_id)).then(|| {
        format!(
...
pub(crate) fn stable_key_segment_is_canonical(value: &str) -> bool {
    icydb_model_legacy::node::stable_key_segment_is_canonical(value)
}
    ...
    fn stable_key_segment_policy_matches_schema_policy() {
        for segment in ["db", "demo_rpg", "store_1", "v1"] {
            assert_eq!(
                ...
                icydb_model_legacy::node::stable_key_segment_is_canonical(segment),
