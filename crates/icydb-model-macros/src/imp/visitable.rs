...
fn field_list(def: &Def, fields: &FieldList) -> TokenStream {
    ...
    let visit_helpers = bindings.iter().map(|binding| {
        ...
        quote! {
            fn #fn_ident(node: &Self, visitor: &mut dyn ::icydb_model::visitor::VisitorCore) {
                use ::icydb_model::visitor::perform_visit;
    ...
    let visit_mut_helpers = bindings.iter().map(|binding| {
        ...
        quote! {
            fn #fn_ident(node: &mut Self, visitor: &mut dyn ::icydb_model::visitor::VisitorMutCore) {
                use ::icydb_model::visitor::perform_visit_mut;
    ...
    let descriptors = bindings.iter().map(|binding| {
        ...
        quote! {
            ::icydb_model::visitor::VisitableFieldDescriptor::new(
                #field_name,
                Self::#visit_fn,
                Self::#visit_mut_fn,
            )
    ...
    let inherent_tokens = Implementor::new(def, TraitKind::Inherent)
        .set_tokens(quote! {
            ...
            const #field_table_ident: &'static [::icydb_model::visitor::VisitableFieldDescriptor<Self>] =
                &[#(#descriptors),*];
        })
        .to_token_stream();
    ...
    let trait_tokens = Implementor::new(def, TraitKind::Visitable)
        .set_tokens(quote! {
            fn drive(&self, visitor: &mut dyn ::icydb_model::visitor::VisitorCore) {
                ::icydb_model::visitor::drive_visitable_fields(visitor, self, Self::#field_table_ident);
            }

            fn drive_mut(&mut self, visitor: &mut dyn ::icydb_model::visitor::VisitorMutCore) {
                ::icydb_model::visitor::drive_visitable_fields_mut(
                    visitor,
                    self,
...
fn enum_variant(variant: &EnumVariant) -> (TokenStream, TokenStream) {
    ...
    if variant.value.is_some() {
        ...
        (
            quote! { Self::#ident(value) => perform_visit(visitor, value, #name), },
            quote! { Self::#ident(value) => perform_visit_mut(visitor, value, #name), },
        )
    } else {
