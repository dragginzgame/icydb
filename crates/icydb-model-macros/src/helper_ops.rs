...
use crate::helper_newtype::{self, NewtypeInput};
...
pub(crate) fn derive_add(input: TokenStream) -> TokenStream {
    derive_op(
        input,
        OpSpec::binary("Add", quote!(::std::ops::Add), "add", quote!(+)),
    )
}
...
pub(crate) fn derive_sub(input: TokenStream) -> TokenStream {
    derive_op(
        input,
        OpSpec::binary("Sub", quote!(::std::ops::Sub), "sub", quote!(-)),
    )
}
...
pub(crate) fn derive_mul(input: TokenStream) -> TokenStream {
    derive_op(
        input,
        OpSpec::binary("Mul", quote!(::std::ops::Mul), "mul", quote!(*)),
    )
}
...
pub(crate) fn derive_div(input: TokenStream) -> TokenStream {
    derive_op(
        input,
        OpSpec::binary("Div", quote!(::std::ops::Div), "div", quote!(/)),
    )
}
...
pub(crate) fn derive_rem(input: TokenStream) -> TokenStream {
    derive_op(
        input,
        OpSpec::binary("Rem", quote!(::std::ops::Rem), "rem", quote!(%)),
    )
}
...
fn derive_op(input: TokenStream, spec: OpSpec) -> TokenStream {
    let newtype = match helper_newtype::parse_newtype(input, spec.label) {
        Ok(newtype) => newtype,
        Err(err) => return err.to_compile_error(),
    };
