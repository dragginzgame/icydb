...
use crate::{RoutedSqlEntity, core_db, db, execute_entity_routed_sql, routed_sql_entity};
use candid::{CandidType, Deserialize};
use icydb::{
    Error,
    db::{
        ExplainExecutionNodeDescriptor, GroupedCountFoldMetrics, MutationResult, PersistedRow,
...
fn generated_query_entity(route: &SqlStatementRoute) -> Result<RoutedSqlEntity, Error> {
    ...
    routed_sql_entity(route.entity()).map_err(|err| {
        invalid_perf_request_error(format!(
            "sql attribution unsupported entity '{}': {}",
            route.entity(),
            err.message()
        ))
    })
...
fn lower_query_for_routed_entity(
    parsed: &icydb_core::db::SqlParsedStatement,
    routed_entity: RoutedSqlEntity,
) -> Result<(), Error> {
    match routed_entity {
        RoutedSqlEntity::Customer => parsed
            ...
            .map(|_| ())
            .map_err(Error::from),
        RoutedSqlEntity::CustomerAccount => parsed
            .lower_query_lane_for_entity(
                CustomerAccount::MODEL.name(),
                CustomerAccount::MODEL.primary_key().name(),
            )
            .map(|_| ())
            .map_err(Error::from),
        RoutedSqlEntity::CustomerOrder => parsed
            .lower_query_lane_for_entity(
                CustomerOrder::MODEL.name(),
                CustomerOrder::MODEL.primary_key().name(),
            )
            .map(|_| ())
            .map_err(Error::from),
        RoutedSqlEntity::SqlWriteProbe => parsed
            .lower_query_lane_for_entity(
                SqlWriteProbe::MODEL.name(),
                SqlWriteProbe::MODEL.primary_key().name(),
            )
            .map(|_| ())
            .map_err(Error::from),
        RoutedSqlEntity::PlannerChoice => parsed
            .lower_query_lane_for_entity(
                icydb_testing_test_sql_parity_fixtures::schema::PlannerChoice::MODEL.name(),
                icydb_testing_test_sql_parity_fixtures::schema::PlannerChoice::MODEL
                    .primary_key()
                    .name(),
            )
            .map(|_| ())
            .map_err(Error::from),
        RoutedSqlEntity::PlannerPrefixChoice => parsed
            .lower_query_lane_for_entity(
                icydb_testing_test_sql_parity_fixtures::schema::PlannerPrefixChoice::MODEL.name(),
                icydb_testing_test_sql_parity_fixtures::schema::PlannerPrefixChoice::MODEL
                    .primary_key()
                    .name(),
            )
            .map(|_| ())
            .map_err(Error::from),
        RoutedSqlEntity::PlannerUniquePrefixChoice => parsed
            .lower_query_lane_for_entity(
                icydb_testing_test_sql_parity_fixtures::schema::PlannerUniquePrefixChoice::MODEL
                    .name(),
                icydb_testing_test_sql_parity_fixtures::schema::PlannerUniquePrefixChoice::MODEL
                    .primary_key()
                    .name(),
...
const fn attribution_phase_split(
    ...
    route_local_instructions: u64,
    lower_local_instructions: u64,
    require_positive_dispatch: bool,
) -> (u64, u64, u64) {
    let attributed_before_execute = parse_local_instructions
        .saturating_add(route_local_instructions)
        .saturating_add(lower_local_instructions);
...
fn attribute_generated_dispatch_surface(sql: &str) -> Result<SqlPerfAttributionSample, Error> {
    ...
    // separate from routed entity resolution and lowered query admission.
    let (parse_local_instructions, parsed_result) =
        measure_result(|| core.parse_sql_statement(sql).map_err(Error::from));
    let parsed = parsed_result?;

    let (route_local_instructions, routed_entity_result) =
        measure_result(|| generated_query_entity(parsed.route()));
    let routed_entity = routed_entity_result?;

    let (lower_local_instructions, lowered_result) =
        measure_result(|| lower_query_for_routed_entity(&parsed, routed_entity));
    lowered_result?;

    // Phase 2: measure the real generated surface total and assign the
    // remaining work to dispatch/execute buckets without reviving the removed
    // generated-dispatch runtime branch.
    let (total_local_instructions, outcome) = measure_surface_call(|| {
        execute_entity_routed_sql(sql)
            .map_or_else(outcome_from_error, outcome_from_sql_query_result)
    });
    let (dispatch_local_instructions, execute_local_instructions, wrapper_local_instructions) =
        attribution_phase_split(
            total_local_instructions,
            parse_local_instructions,
    ...
    E: PersistedRow<Canister = SqlParityCanister> + EntityValue,
{
    ...
    // cost across dispatch and execute without depending on removed typed
    // dispatch wrappers.
    let (total_local_instructions, outcome) = measure_surface_call(|| {
        db().execute_entity_sql::<E>(sql)
            .map_or_else(outcome_from_error, outcome_from_sql_query_result)
    });
    let (dispatch_local_instructions, execute_local_instructions, wrapper_local_instructions) =
        attribution_phase_split(
