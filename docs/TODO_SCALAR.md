...
- `ValueFamily` + `ValueFamilyExt` mapping:
...
- `FieldType::family` (`ScalarType::family`) and `Value::family` in coercion validation:
  - `crates/icydb-core/src/db/query/predicate/validate.rs:713-739`
...
Columns:
- Family and matches from `ScalarType::family` and `ScalarType::matches_value`.
...
### Asymmetries
- `Date`, `IntBig`, `UintBig` are Numeric family in `ScalarType::family` and `Value::family`, but excluded from `Value::is_numeric` (`value/mod.rs:131-148`).
- Numeric coercion is gated by family, but numeric comparison uses `cmp_numeric` which excludes `Date` (`value/mod.rs:223-277`).
- `ValueFamily` docs list Identifier includes Subaccount, but implementation classifies Subaccount as Blob (`value/family.rs:19-25`, `value/mod.rs:562-564`).
...
- Every scalar must be updated in:
  ...
  - `ValueFamilyExt for Value`
  - numeric classification paths if applicable
- `Value::family` and `ScalarType::family` must remain aligned for equivalent variants.
...
1. Numeric classification drift:
   - `Value::is_numeric` vs `ValueFamily::Numeric` vs `numeric_repr`.
2. Keyability split:
...
- `Value::is_numeric`
- (optional) `ValueFamilyExt for Value` for scalar variants
...
1. Add registry + generate `ScalarType::family` and `ScalarType::matches_value`.
2. Generate `field_type_from_model_kind` for scalar kinds.
3. Generate `Value::is_numeric` from registry.
4. Optionally generate `ValueFamilyExt::family` for scalar variants.
...
- For each scalar:
  ...
  - `ScalarType::family` matches `Value::family` for the same variant.
- `Value::is_numeric` matches the current list exactly.

### Behavior that must remain identical
- `ScalarType::family` and `ScalarType::matches_value` semantics.
- `field_type_from_model_kind` mapping for all scalar kinds.
- `Value::family` output per variant.
...
- Align `Value::is_numeric` with `ValueFamily::Numeric` by adding `Date`, `IntBig`, `UintBig`.
