# 0.22 — Streaming Aggregates (COUNT / MIN / MAX / EXISTS)
...
* `COUNT` pushdown optimizations (tracked as post-`0.22.0` minor work)

---

## 4️⃣ Aggregate Semantics
...
## 1️⃣2️⃣ Complexity Impact

Expected effect:

* reduces duplicated “materialize then aggregate” work
...
## 1️⃣3️⃣ Post-`0.22.0` Minor Release Schedule

Planned follow-up releases after `0.22.0`:

* `0.22.1`: finish `COUNT` pushdown for clearly safe plans, and keep it gated behind shared streaming safety.
* `0.22.2`: add `COUNT` pushdown parity and regression matrix coverage (fast path vs fallback, ASC/DESC, DISTINCT on/off).
* `0.23.0`: move physical access paths from vector materialization to iterator-backed ordered streams.
* `0.24.0`: add aggregate-aware fast-path variants for `COUNT`/`MIN`/`MAX` where semantics are provably equivalent.

Pushdown safety rule:

```text
count_pushdown_eligible ⊆ streaming_aggregate_eligible
```

`COUNT` pushdown must reuse the same streaming safety predicate and then add stricter checks.
It must never introduce a parallel or broader eligibility decision.
