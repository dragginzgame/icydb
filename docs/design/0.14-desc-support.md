...
This must hold for deterministic datasets without duplicate ordered values.

Tie-group policy:

* When ordered values are equal, PK tie-break remains canonical and stable.
* DESC does not invert intra-group PK order.
* Duplicate-group correctness is enforced via continuation monotonicity and
  boundary exclusion semantics, not strict row-level reversal.
...
Use deterministic datasets with unique ordered values for strict row-level
...
* duplicate-group continuation with canonical PK tie-break stability
