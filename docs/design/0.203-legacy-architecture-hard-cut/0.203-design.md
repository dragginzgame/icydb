...
| Is there exactly one cursor contract? | **Yes.** | One bounded v2 binary codec and validation spine; active cursor/query contracts describe route-dependent continuation without an old decoder. |
...
| 9 — Active docs and terminology | Complete (2026-07-14) | Cursor docs describe the bounded v2 binary wire and route-dependent continuation; archive links, read-admission surfaces, and grouped terminology match current code; focused docs/source searches pass. |
...
| Changed-package compilation and lint | PASS | `icydb-core`, `icydb`, `icydb-cli`, and `icydb-diagnostic-code` pass warnings-denied all-target/all-feature Clippy; focused facade/core/CLI checks also pass. |
| Query, planner, executor, and admission tests | PASS | Focused intent, SQL lowering, pipeline, admission, read-admission, cursor, grouped-plan, route, metrics, projection, and attribution suites pass. |
...
| Representative raw Wasm | PASS | `ten_entity_fluent_rows` wasm-release raw artifact decreased from 2,096,898 to 2,096,342 bytes (−556); deterministic gzip decreased from 737,533 to 737,466 bytes (−67). The canonical shrunk artifact decreased from 1,950,484 to 1,950,129 bytes (−355; gzip −377). |

## Implementation Change and Cost Readout

- Changed inventory: **160 files** including the new maintained trusted-SQL
  compile example and detailed release notes; **2,081 insertions and 4,848
