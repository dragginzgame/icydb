...
Complete. The broad lexical cleanup is closed; future renames require the
risk-based threshold in `0.165-design.md`.
...
No unclassified live old-name drift remains from the accepted 0.165 renames.

Future families may still audit narrower concepts, but remaining generic
vocabulary should be renamed only when it creates real semantic or maintenance
risk:

- ownership or boundary confusion
- hidden lifecycle, state, or phase
- misleading call sites
- public, generated, or cross-crate API ambiguity
- poor grepability for an architectural concept
- risk of using the wrong abstraction

Otherwise, local leaf-helper names and conventional domain vocabulary are kept.
