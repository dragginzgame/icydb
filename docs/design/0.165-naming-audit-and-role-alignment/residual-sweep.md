...
```bash
rg -n "LoadOrderRouteContract|GroupedExecutionModeProjection|RouteCapabilities|route::capability\\b|route::contracts::capabilities|CoveringProjectionContext|CoveringAccessMetadata|PreparedExecutionPlanCoreShared|PreparedExecutionInputParts|GroupedPathRuntimeCore|PlannedCursor|GroupedPlannedCursor|ScalarTokenParts|GroupedTokenParts|GroupedContinuationToken::into_parts|PreparedSqlScalarAggregateDescriptorShape|SqlGlobalAggregateCommandCore|compile_sql_global_aggregate_command_core_from_prepared_with_schema|AggregateTerminalSemantics|resolve_or_insert_global_aggregate_terminal|resolve_having_global_aggregate_terminal_index" crates docs CHANGELOG.md
```
