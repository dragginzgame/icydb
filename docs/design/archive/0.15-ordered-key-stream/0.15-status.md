...
This status tracks progress against `docs/design/archive/0.15-ordered-key-stream/0.15-design.md`
...
- Wiring two-stream merge helpers into composite execution paths
...
- [x] Changelog merged into one `0.15.0` entry (includes the merge-stream add-on)
...
- Done:
  ...
  - Added internal `MergeOrderedKeyStream` helper for ordered two-stream merge
    with duplicate suppression (extra groundwork, not required for core 0.15).
  - Added focused unit tests for stream ordering and exhaustion behavior.
- Remaining:
  - `MergeOrderedKeyStream` is not wired into `Union`/`Intersection` execution
    in 0.15 (still intentionally out of scope).
...
- `cargo clippy -p icydb-core --all-targets --locked -- -D warnings` is blocked
  in that environment by a local build issue, not code-level lint findings.
...
- Semantic ownership remains unchanged (cursor/limit/post-access still owned by
  existing executor/cursor layers), which matches the design’s non-relocation
  principle.
- Existing behavior and regression baselines remained stable through the change.
- The extra merge-stream helper is currently additive only; it does not change
  ownership of continuation, limit, boundary validation, predicate filtering,
  or row deserialization.
