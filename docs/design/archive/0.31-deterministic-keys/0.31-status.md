...
- `docs/design/archive/0.31-deterministic-keys/0.31-design.md`
...
Current `0.31.0` status: **in progress (release gate pending)**.
...
4. Canonical key/equality substrate for aggregate distinct paths:
   ...
   - digest- and stable-hash upgrade vectors plus seed/version freeze tests now lock hash behavior.
...
- [x] Lock fixed-seed and upgrade-stability contracts
- [x] Add hash/canonical-equality invariance tests
...
- `cargo test -p icydb-core hash_digest_contract_vectors_are_frozen_for_upgrade_stability --locked` (pass)
- `cargo test -p icydb-core hash_contract_seed_and_version_are_frozen --locked` (pass)
...
- `cargo test -p icydb-core canonical_equal_keys_always_share_stable_hash --locked` (pass)
...
- `cargo clippy -p icydb-core --all-targets --locked -- -D warnings` (blocked in that environment by a local build issue)
