...
Status: implementation complete; all seventeen hard-cut slices are applied and
focused closeout validation passes.
...
The audited repository has fourteen proven hard-cut findings:
...
13. admission exposes policy and diagnostic facts that no runtime can
    produce;
14. SQL diagnostics publicly rename the canonical scalar-aggregate
    attribution type through an invariant-free alias.
...
| Recovery | Database format admission, commit recovery, schema/index recovery | `db/database_format`, `db/commit/recovery`, schema/index recovery owners | `PRE_BOOT_STABLE_CELL_MAGIC` is a fail-closed overwrite guard, not a legacy decoder |
...
| DDL | `execute_admin_sql_ddl` / generated controller DDL | Catalog-native schema mutation | SQL is syntax only; accepted schema publication owns mutation | DDL status/result | Retain; no SQL-as-authority path found |
...
### Admission diagnostics report only enforceable facts

Visibility narrowing exposed policy fields and diagnostic variants that no
production path could set or produce. 0.203 deletes the unused scan,
materialized-row, projection-column, and response-byte policy fields; the
always-absent response-byte summary fields; the unproduced estimate and unknown
states; and eight rejection codes with no runtime producer.

The surviving built-in policy enforces returned-row bounds, index/full-scan
requirements, materialized-sort rejection, primary-key input bounds, and
grouped budgets. Ordinary fluent reads cannot construct OFFSET intent; trusted
SQL and internal planner/executor contracts retain OFFSET without routing it
through public-read admission. Application endpoints remain responsible for
any byte bound on their final application-shaped response.
This is a runtime-neutral hard cut: the deleted knobs never affected an
admission decision. Public diagnostic numbers after the removed entries are
compacted because pre-1.0 does not preserve retired wire values.
...
`docs/contracts/CURSOR.md` is rewritten in place as the concise current wire
and implementation contract aligned with normative `QUERY_CONTRACT.md`. Its
historical checkpoint prose is deleted rather than retained beside the current
contract. It must not claim that current cursors use CBOR or cannot push
continuation into an eligible ordered route.
...
| HCLA-013 | Medium | A + D — Delete/re-document | Admission / errors / diagnostics | Internal visibility had shielded no-op policy knobs, always-empty response-bound fields, eight rejection codes with no producer, and a test-only fluent OFFSET method | `QueryAdmissionPolicy` could never set scan/materialization/projection limits; response-byte bounds were always `None`; compiler reachability after privatization identified unconstructed variants; production fluent construction had no OFFSET method | Diagnostics promised rejection modes that runtime could not emit, while tests preserved a fluent pagination mode absent from the product | Delete the no-op fields, impossible states/codes, test-only OFFSET hook, stale CLI help, and contract text; retain only enforced admission facts and trusted SQL/internal OFFSET semantics |
| HCLA-014 | Low | A — Delete now | Diagnostics / public API | SQL diagnostics publicly alias the canonical scalar-aggregate attribution type under a second name | `SqlScalarAggregateAttribution` is exactly `ScalarAggregateAttribution`; the alias has no distinct producer or invariant and both names are re-exported | Two names imply frontend-specific ownership for one executor fact and expand the public surface without a use case | Delete the alias and use `ScalarAggregateAttribution` directly in `SqlQueryExecutionAttribution` |
| HCLA-015 | High | A + B — Delete/collapse | Schema mutation / planning | Three successive plan representations and a generic capability-negotiation shell survive around the concrete mutation runners | `MutationPlan` lowered to `SchemaRebuildPlan`, then `SchemaMutationExecutionPlan`; `SchemaMutationRunnerContract` advertised future capabilities; most rejection/outcome state was test-only or had no current producer | A new mutation can attach semantics to the request, rebuild, execution, or runner-contract layer and produce competing publication truth | Delete the generic execution module and capability shell; carry one canonical `MutationPlan` from transition policy into the concrete runner |
| HCLA-016 | Medium | B — Collapse into current authority | Schema diagnostics / metrics | Current transition plans for index additions and metadata rename are recorded as `ExactMatch` | `schema_transition_plan_outcome` grouped four distinct `SchemaTransitionPlanKind` values into `SchemaTransitionOutcome::ExactMatch` | Metrics disagree with the plan selected for execution and hide which physical or metadata route reconciliation attempted | Give every current transition kind a matching metric bucket and derive it directly from `SchemaTransitionPlanKind` |
| HCLA-017 | High | A + B — Delete/collapse | Schema mutation / publication | The field-path physical runner still reported runtime invalidation and accepted-snapshot publication through production no-op sinks before the real publication gate ran | Sink comments said they existed before real wiring; startup passed empty implementations and later published through `SchemaPublicationGate` | Duplicate publication truth could report completion early and left physical target keys behind when the real gate rejected | Delete the fake phases, sink traits, runtime-epoch identity, and no-op adapters; roll back physical work on later publication failure |
| HCLA-018 | Medium | B — Collapse into current authority | Runtime schema authority | Production accepted `SchemaInfo` construction could omit the accepted catalog and copy generated field kinds into a hybrid view | Commit preparation and plan-cache reconstruction used the catalog-less constructor; `has_accepted_authority` depended on catalog presence | New runtime semantics could silently attach to generated facts despite an accepted snapshot being present | Require catalog-backed production construction and retain generated kinds only in explicitly test/model-only views |
| HCLA-019 | Medium | A + C — Delete/privatise | Cursor and admin-batch API | Raw cursor byte codecs and two redundant `AdminBatchRequest` cursor methods remained public without an application use case | Facade responses used core `encode_cursor`; no external caller used `decode_cursor`, `with_cursor`, or `cursor` | Applications could bind to internal token representation and duplicate continuation spellings | Keep opaque string cursors at the facade, privatise raw decoding, and retain only `AdminBatchRequest::new` / `next` |
| HCLA-020 | High | B — Collapse into current authority | SQL DDL / publication atomicity | `DROP COLUMN` rewrote dense rows and `DROP INDEX` removed/remapped physical keys before fallible accepted-schema publication without rollback | `execute_admin_sql_ddl_field_drop` inserted rewritten rows before `SqlDdlPublicationEnvelope::publish`; `execute_admin_sql_ddl_secondary_index_drop` mutated the index store before the same boundary | Publication rejection could leave accepted-before metadata authoritative over accepted-after physical state | Retain exact row/index before-images through publication and restore them on mutation or publication failure |
...
- `crates/icydb-core/tests/ui/db/executor/grouped_route_observability_module_private.rs`
  and `.stderr` — compile-fail fixture naming the obsolete type.
...
The core UI suite also names `GroupedRouteObservability` solely to prove it is
not public. Those tests currently preserve the false model rather than a
...
unless a real alternate grouped route is introduced now. Delete the compile-fail
fixture and its architecture-guard row with the retired type.
...
- `crates/icydb-core/src/db/cursor/token/codec.rs` — actual custom bounded
  binary `TOKEN_WIRE_VERSION: u8 = 1` implementation.
...
### HCLA-013 — Admission exposed facts that no runtime could produce

**Files and symbols**

- `crates/icydb-core/src/db/query/admission/{policy.rs,plan_summary.rs}` —
  unset scan/materialization/projection caps, an ignored response-byte cap, and
  response-byte summary fields that were always `None`.
- `crates/icydb-core/src/db/query/admission.rs` — unconstructed
  `EstimateOnly` / `Unknown` states and rejection variants backed only by the
  inert fields.
- `crates/icydb-diagnostic-code/src/{lib,registry}.rs` and
  `crates/icydb-cli/src/diagnostic.rs` — public codes and help text for those
  unproduced rejections.
- `crates/icydb-core/src/db/query/fluent/load/builder.rs` — a `cfg(test)`
  OFFSET method that let tests preserve a fluent mode absent from production.

**Current behavior and reachability.** The live public policy enforces returned
rows, index/full-scan shape, materialized-sort prohibition, primary-key input
work, and grouped budgets. Reference and constructor searches showed no
producer for scan-budget, materialization-budget, projection-column,
estimated-bound, response-byte-bound, or lane-introspection rejection state.
The response-byte constructor argument was stored but never consulted. A final
compiler-driven reachability pass also proved that `ScanBoundUnavailable`,
`UnsupportedStatementForQueryLane`, and `PublicQueryOffsetRejected` had no
runtime producer: missing public bounds already reject as
`PublicQueryRequiresLimit`, statement-lane mismatches are excluded before this
policy, and production fluent construction cannot create OFFSET intent.

**Historical behavior represented.** These fields describe a configurable
policy framework and plan-level byte proof that the current built-in admission
architecture never completed and no longer exposes.

**Compatibility.** There is no persisted format. The internal DTOs have no
public consumer. Eight public diagnostic variants and their raw numbers are
source/wire visible, but pre-1.0 hard-cut rules do not preserve errors that have
no producer. Trusted SQL OFFSET semantics remain current and unchanged.

**Recommended end state, applied.** Delete the inert fields, always-empty
diagnostic properties, impossible states, dead constructors/helpers, eight
unproduced rejection codes, and the test-only fluent OFFSET method. Compact the
remaining diagnostic registry and make the read-admission contract describe
the enforced policy only. Keep OFFSET tests at the trusted SQL and internal
planner/executor boundaries that actually own the behavior.

**Semantic impact.** None in production runtime behavior; no removed branch or
fluent method could compile outside tests. Public error vocabulary contracts to
reachable failures, and tests no longer imply that fluent OFFSET is supported.

**Focused validation.** Admission policy/session tests, structured/text
EXPLAIN tests, diagnostic-code registry tests, CLI rendering tests, and the
read-admission invariant check, plus trusted SQL and internal OFFSET tests.

### HCLA-014 — SQL scalar attribution duplicated the canonical type name

**Files and symbols.** `crates/icydb-core/src/db/session/sql/attribution.rs`
declared `SqlScalarAggregateAttribution` as a public type alias for
`crate::db::ScalarAggregateAttribution`; SQL attribution, core session/root
re-exports, and the facade all propagated both names.

**Current behavior and reachability.** The alias had no constructor, fields,
wire distinction, or semantic owner of its own. Its only value-bearing use was
the `SqlQueryExecutionAttribution::scalar_aggregate` field, populated by the
canonical executor attribution constructor.

**Historical behavior represented.** The SQL prefix reflects a frontend-local
diagnostic vocabulary from before fluent and SQL aggregate execution shared
one attribution fact.

**Compatibility.** This is source naming only; the aliased Candid/Serde shape
is unchanged. Pre-1.0 hard-cut rules do not preserve the duplicate spelling.

**Recommended end state, applied.** Delete the alias and all re-exports. Type
the SQL attribution field directly as `ScalarAggregateAttribution`.

**Semantic impact.** None at runtime or on the encoded attribution payload.

**Focused validation.** Diagnostics/all-feature compilation and Clippy, SQL
attribution tests, and public-name residue search.

### HCLA-015 — Schema mutation carried three competing plan contracts

**Files and symbols.** `crates/icydb-core/src/db/schema/mutation/mod.rs`
defined `MutationPlan` and `SchemaRebuildPlan`; the deleted
`mutation/execution.rs` converted the latter into
`SchemaMutationExecutionPlan` and wrapped it in
`SchemaMutationRunnerContract` capability negotiation. `mutation/runner.rs`
then copied that execution plan into `SchemaMutationRunnerInput`. The surviving
metadata plan also retained a per-field `SchemaMutation` vector read only by
tests, while field-path “developer report” fields and an expression runner
report adapter existed only under `cfg(test)`.

**Current behavior and reachability.** Concrete field-path and expression
staging already require their accepted target types. Metadata-only transitions
publish immediately; index additions require the matching physical runner.
Secondary-index drop is a live SQL DDL cleanup target but never needed the
generic rebuild-plan route. Full rewrite, generic capability failures, and
several runner outcome variants had no current producer or existed only in
tests.

**Historical behavior represented.** The layers model a future pluggable
schema runner that negotiates arbitrary rebuild capabilities. Current IcyDB
instead has catalog-native transition policy, concrete physical runners, and a
closed publication preflight.

**Recommended end state, applied.** Delete `mutation/execution.rs`,
`SchemaRebuildPlan`, `SchemaMutationExecutionPlan`, capability contracts, and
unsupported pseudo-plan variants. Delete test-only diagnostic projections and
the unconsumed metadata mutation vector. Make `MutationPlan` the sole closed
plan passed from `SchemaMutationRequest` and `SchemaTransitionPlan` into runner
input. Keep secondary-index drop cleanup on its concrete SQL DDL path.

**Semantic impact.** Maintained metadata, field-path index, expression-index,
and SQL drop-index behavior is unchanged. Impossible/future states can no
longer be constructed, and publication still fails closed whenever the
matching physical path has not completed.

**Focused validation.** Mutation planning/staging/runner tests, transition
policy tests, reconciliation tests, SQL DDL test compilation, and deleted-name
reference search.

### HCLA-016 — Transition metrics collapsed live plans into exact match

**Files and symbols.** `crates/icydb-core/src/db/schema/reconcile.rs` mapped
`AddExpressionIndex`, `AddFieldPathIndex`, and `MetadataOnlyIndexRename` to
`SchemaTransitionOutcome::ExactMatch`; the metrics event, counter, operation,
entity-summary, and tests exposed only exact-match and append-only positive
buckets.

**Current behavior and reachability.** Reconciliation selects each of those
plan kinds today. Field-path addition executes a physical rebuild, expression
addition remains fail-closed where its matching startup path is unavailable,
and metadata rename publishes changed accepted metadata. Recording all three
as exact match hides execution truth.

**Recommended end state, applied.** Add one positive metric outcome for every
current `SchemaTransitionPlanKind` and map the plan kind directly. Remove the
old impossible extraction branch used after matching an accepted plan.

**Semantic impact.** Schema execution and publication do not change. Metrics
now distinguish current transition decisions, and new Candid metric fields are
part of the pre-1.0 hard cut without aliases for the misleading aggregate.

**Focused validation.** Transition metric accumulation, plan-to-metric mapping,
field-path reconciliation, metrics DTO compilation, and focused schema tests.

### HCLA-017 — Physical runners claimed publication they did not perform

**Files and symbols.** The deleted
`schema/mutation/field_path/publication.rs` defined runtime-invalidation and
accepted-snapshot publication plans, reports, and sink traits. Startup supplied
empty sink implementations, while `StartupFieldPathPublicationDecision` later
called the real `SchemaPublicationGate`.

**Current behavior and reachability.** The fake phases were on every field-path
index addition. Physical keys were made ready before the real publication gate,
and rejection after that point had no owner for rolling back those keys.
Expression-index additions had the same later-failure cleanup gap without the
fake sink layer.

**Recommended end state, applied.** The physical runner now owns only staging,
physical validation, the transition back to ready state, and a rollback plan.
Accepted snapshot publication remains solely in `SchemaPublicationGate`.
`SchemaMutationStoreVisibility` and its manually manufactured impossible-state
tests are deleted. The fixed `SchemaMutationRunnerPhase` trace and duplicate
`SchemaFieldPathIndexMutationProgress` copy are also gone. Test-only staged
write/discard/rollback report shells and the one-field runner-failure wrapper no
longer duplicate facts already proven by the writer, typed runner error, and
staged validation; proof-bearing validation and actual `IndexStore` state carry
physical truth. Field-path and expression additions undo their own target keys
when row, physical, or accepted publication validation subsequently fails.

**Semantic impact.** Successful index additions are unchanged. Rejected
publication no longer leaves the failed attempt's physical keys behind, and
runner diagnostics cannot claim runtime invalidation or schema publication.

**Focused validation.** Field-path staging/runner/store tests, reconciliation
publication rejection tests, expression batch rollback tests, and SQL DDL
compilation.

### HCLA-018 — Accepted `SchemaInfo` retained generated authority

**Files and symbols.** `SchemaInfo` accepted-snapshot construction accepted an
optional enum catalog, copied generated `FieldKind` and nested models into the
same field table, and inferred accepted authority from catalog presence.
Commit preparation and query-plan cache reconstruction used the catalog-less
constructor in production.

**Recommended end state, applied.** Production accepted construction requires
an `AcceptedValueCatalogHandle`; commit preparation takes it from the selected
catalog root and plan-cache reconstruction takes it from `EntityAuthority`.
Accepted instances do not copy generated field kinds or nested models. The
catalog-less constructor is test-only and names its generated-model role.

**Semantic impact.** Current accepted behavior remains unchanged. Missing
catalog authority now fails closed instead of producing a hybrid runtime view.

**Focused validation.** Accepted schema-info tests, commit preparation,
predicate/type-inference tests, query-plan cache tests, and SQL lowering.

### HCLA-019 — Raw cursor and duplicate admin continuation APIs leaked

**Files and symbols.** `icydb_core::db` publicly re-exported raw cursor hex
encode/decode functions. `AdminBatchRequest` exposed both `next(cursor)` and
the unused `with_cursor`/`cursor` pair.

**Recommended end state, applied.** Cursor bytes remain internal to core;
facade response construction formats the opaque token through the hidden
generic hex support boundary. Raw cursor decoding is crate-private.
`AdminBatchRequest` retains only `new()` and `next(cursor)`.

**Semantic impact.** Cursor wire bytes and continuation behavior are unchanged.
Only unneeded pre-1.0 surface is removed.

**Focused validation.** Cursor codec tests, fluent page/admin-batch tests,
facade response compilation, and public-name residue search.

### HCLA-020 — Destructive SQL DDL lacked publication rollback

**Files and symbols.** `execute_admin_sql_ddl_field_drop` rewrote every target
entity row to the compacted layout before calling the fallible accepted-schema
publication boundary. `execute_admin_sql_ddl_secondary_index_drop` removed the
target namespace and remapped higher ordinals before the same boundary.

**Current behavior and reachability.** Both paths are the live admin SQL DDL
routes for `DROP COLUMN` and `DROP INDEX`. A publication identity rejection
left the accepted-before schema unchanged but previously had no physical
before-image to restore.

**Recommended end state, applied.** Field drops retain the exact stored row
bytes beside each rewritten row and restore them if rewrite validation or
publication fails. Index drops validate the full remove/remap set before
mutation, retain every original key/value pair, and restore that set after a
partial mutation or rejected publication. Accepted schema publication remains
the sole commit point.

**Semantic impact.** Successful destructive DDL is unchanged. Rejected DDL is
now atomic with respect to accepted metadata and physical row/index state.

**Focused validation.** Publication-race tests compare exact row bytes and the
complete physical index entry set before and after rejection; existing
successful dense-row and index-ordinal compaction tests remain authoritative.
...
- `crates/icydb-core/src/db/schema/mutation/field_path/publication.rs` in full;
  physical ready-state ownership moves to `physical_store.rs`.
- `crates/icydb-core/src/db/schema/mutation/identity.rs` in full.
- `crates/icydb-core/src/db/schema/mutation/execution.rs` in full.
...
- `crates/icydb-core/tests/ui/db/executor/grouped_route_observability_module_private.rs`.
- `crates/icydb-core/tests/ui/db/executor/grouped_route_observability_module_private.stderr`.
...
- `SchemaMutationRuntimeInvalidationSink` and
  `SchemaMutationAcceptedSnapshotPublicationSink`.
- `SchemaFieldPathIndexRuntimeInvalidationPlan` / `Report` and
  `SchemaFieldPathIndexSnapshotPublicationPlan` / `Report` / `Error`.
- `SchemaMutationRuntimeEpoch` and `SchemaMutationPublicationIdentity`.
- `SchemaMutationStoreVisibility` and visibility-only rejection variants.
- `SchemaMutationRunnerPhase` and `SchemaFieldPathIndexMutationProgress`.
- `SchemaFieldPathIndexStagedStoreWriteReport` and
  `SchemaFieldPathIndexStagedDiscardReport`.
- `SchemaFieldPathIndexStagedStoreRollbackReport` and the one-field
  `SchemaFieldPathIndexRunnerFailure` wrapper.
- `PlannedQuery<E>`.
- `CompiledQuery<E>`.
...
- Catalog-less production `SchemaInfo` accepted-snapshot constructors.
- Public `db::decode_cursor` and `db::encode_cursor` re-exports.
- `AdminBatchRequest::with_cursor` and `AdminBatchRequest::cursor`.
- The test-only `SchemaMutationRunnerInput::accepted_before` accessor and its
  retained post-validation field.
...
- Core/facade re-exports of `SqlScalarAggregateAttribution`.
...
- `SchemaRebuildPlan`, `SchemaMutationExecutionPlan`,
  `SchemaMutationRunnerContract`, and `SchemaMutationRunnerCapability`.
- Generic runner rejection/outcome/no-op adapter state used only to preserve
  future capability negotiation.
- Unsupported `FullDataRewrite`, incompatible-request, and mutation-plan
  secondary-drop pseudo-states. The concrete SQL DDL drop cleanup target
  remains current.
- The test-only expression runner-report adapter, synthetic field-path
  developer-report fields, and unconsumed per-field `SchemaMutation` payload.

### Tests and fixtures

- Field-path runner tests that record no-op invalidation/publication sinks or
  assert fake publication phases.
- Runtime-epoch and publication-identity tests for the deleted mutation bridge.
...
| Scalar aggregate attribution | Canonical `ScalarAggregateAttribution`; SQL-prefixed public alias | Executor-owned `ScalarAggregateAttribution` projected by either frontend | Delete SQL alias (HCLA-014) |
...
| Schema mutation plan | `MutationPlan`; derived `SchemaRebuildPlan`; derived `SchemaMutationExecutionPlan`; runner capability contract | Closed schema-owned `MutationPlan` consumed by concrete runners | Delete conversions and capability shell (HCLA-015) |
| Schema transition identity in metrics | `SchemaTransitionPlanKind`; coarse positive metric bucket that labels multiple kinds exact-match | `SchemaTransitionPlanKind`, projected one-to-one into metrics | Add current positive buckets and direct mapping (HCLA-016) |
| Accepted snapshot publication | Field-path runner sink/report; real `SchemaPublicationGate` | `SchemaPublicationGate` after physical validation | Delete fake sink phases and roll back rejected physical attempts (HCLA-017) |
| Runtime schema field authority | Accepted snapshot fields plus generated `FieldKind` in catalog-less `SchemaInfo` | Accepted snapshot and accepted enum catalog | Require catalog-backed production construction (HCLA-018) |
| Cursor token representation | Internal byte codec plus normal public raw encode/decode exports | Cursor module internally; opaque string at facade boundary | Privatise raw codec and remove duplicate admin cursor spellings (HCLA-019) |
| Destructive DDL commit state | Accepted schema publication; pre-publication row rewrite or index remove/remap | Accepted schema publication as commit point, with operation-owned physical before-image | Roll back physical work on every rejected publication (HCLA-020) |
| Cursor implementation truth | Custom binary v1 codec and route-dependent pushdown; historical CBOR/no-pushdown “Current Behavior” | Cursor codec/validation plus normative query contract | Archive/rewrite checkpoint (HCLA-012) |
...
fluent reads do not expose it, including in test builds. Internal query and
executor contracts retain offset state solely to implement trusted SQL and to
test that physical execution behavior. This is a semantic lane distinction,
not a compatibility path.

### Cursor wire version and rejection corpus

`TOKEN_WIRE_VERSION = 1` is the sole accepted cursor wire. The decoder checks
...
### Database pre-boot stable-cell guard

`PRE_BOOT_STABLE_CELL_MAGIC` in `db/database_format/mod.rs` does not decode an old
...
| --- | --- | --- |
| Is there exactly one ordinary typed/fluent query architecture? | **Yes.** | Public construction and diagnostics use fluent builders; `AccessPlannedQuery` and `SharedPreparedExecutionPlan` are the sole internal logical and executor-ready stages. |
| Is there exactly one SQL architecture? | **Yes.** | Production and tests use the same SQL compile/execute system; the test-only public-SQL admission shadow was deleted in Slice 2. |
...
| Is there exactly one owner of public-read admission? | **Yes.** | Fluent built-in policy is the live owner; the SQL test duplicate was deleted in Slice 2. |
...
| Is there exactly one cursor contract? | **Yes.** | One bounded `ICYQ` v1 binary codec and validation spine; active cursor/query contracts describe route-dependent continuation without an old decoder. |
...
| Is there exactly one authoritative route-selection result? | **Yes.** | `ExecutionRoutePlan` selects execution; Slice 4 deleted the manufactured grouped eligibility/outcome/rejection state. |
| Do diagnostics consume execution truth rather than reconstruct it? | **Yes for planner candidate and grouped-route facts.** | Slices 3–4 carry typed planner candidate/rejection facts and actual grouped execution mode/fallback reason into projections. |
| Are trusted/admin bypasses explicit and isolated? | **Yes.** | `trusted_read_unchecked()` and trusted SQL names are explicit; generated SQL remains controller-gated. |
| Does each public type correspond to a current developer use case? | **Yes for the audited query/admission/response/diagnostic surface.** | Admission DTOs, inert query stages, raw facade `Query`, dead response wrappers, and duplicate aliases are gone. |
| Are obsolete compatibility promises absent? | **Yes for the audited surface.** | SQL/CLI aliases and anti-resurrection fixtures were deleted without replacements. |
| Are all alternate execution routes justified by semantics rather than history? | **Yes.** | Shadow SQL admission and fictitious grouped-route observability were deleted; actual grouped hash/ordered and scalar optimized/materialized routes retain distinct semantics. |
| Can a new contributor identify the current path without understanding prior pivots? | **Yes for the audited architecture.** | Fluent/SQL entry, internal plan stages, admission ownership, projected output, cursor wire, and trusted bypasses now have one documented path. |
...
- `ProjectionResponse`, `ProjectionRows`, `SqlProjectionRows`, and
  `SqlQueryRowsOutput`;
- the duplicate `SqlScalarAggregateAttribution` diagnostic alias;
...
- the database boot record and `PRE_BOOT_STABLE_CELL_MAGIC` fail-closed
  overwrite guard;
...
- one current cursor wire with `TOKEN_WIRE_VERSION = 1`;
...
| --- | --- | --- |
| 1 — Trusted SQL authority | Complete (2026-07-14) | Facade/core/generated consumers use only `execute_trusted_sql_query` and trusted attribution names; generated controller guard is ordered before dispatch; phantom admin/development lanes and their test policy are deleted; focused facade, generator, SQL, admission, CLI, canister, and architecture validation passes. |
| 2 — Shadow public-SQL admission | Complete (2026-07-14) | Test-only SQL policy entry/dispatch, SQL-specific response budgeting, grouped-cap mutation helpers, and all 25 public-SQL admission tests are deleted; fluent admission, trusted SQL, SQL lowering, generated controller gates, and architecture checks pass. |
| 3 — Typed diagnostic execution truth | Complete (2026-07-14) | Planner snapshots carry typed index candidate identity and typed rejection causes; logical and executor diagnostics render from those facts without label parsing, selected-label matching, or a sole-candidate fallback; focused planner, text/JSON EXPLAIN, verbose route, and Clippy validation passes. |
| 4 — Grouped route truth | Complete (2026-07-14) | Diagnostics consume the selected hash/ordered mode and real planner fallback reason; fictitious grouped eligibility/outcome/rejection state and its obsolete UI fixture are deleted; grouped route, cursor, EXPLAIN, metrics, and parity tests pass. |
| 5 — Fallback and compatibility machinery | Complete (2026-07-14) | Generated fallback metrics, their one-variant wrapper, the `primary_key` CLI alias, and compatibility-only UI fixtures are deleted; accepted-schema, metrics, CLI, macro, and core UI checks pass. |
| 6 — Query stages and raw facade query | Complete (2026-07-14) | `PlannedQuery`, `CompiledQuery`, `QueryPlanHandle`, `.planned()`, `.plan()`, raw facade `Query`, and raw-query wrappers are deleted; intent, SQL lowering, pipeline snapshots, facade compile, and generator checks pass. |
| 7 — Response/projection vocabulary | Complete (2026-07-14) | SQL and fluent returning carry `RowProjectionOutput` directly; dead projection responses, intermediate rows, SQL projection aliases, the duplicate SQL scalar-attribution alias, clone-back conversion, and stale guide text are deleted; facade SQL and CLI rendering checks pass. |
| 8 — Admission internals | Complete (2026-07-14) | Eleven admission DTOs are internal-only; no-op policy knobs, always-empty diagnostics, eight impossible rejection codes, a test-only fluent OFFSET hook, and a dead public core helper are deleted; admission, diagnostic registry, CLI, trusted SQL/internal OFFSET, and invariant checks pass. |
| 9 — Active docs and terminology | Complete (2026-07-14) | Cursor docs describe the sole bounded `ICYQ` v1 binary wire and route-dependent continuation; archive links, read-admission surfaces, and grouped terminology match current code; focused docs/source searches pass. |
| 10 — Mutation/schema/response excavation | Complete (2026-07-14) | Dead `MutationResult` and projected-row facades, neutral executor aliases, unreachable write-policy modes, and unsupported schema-mutation pseudo-states are deleted; current domain results and executable mutation plans remain. |
| 11 — Fluent/runtime authority hard cut | Complete (2026-07-14) | The hidden fluent raw-limit hook and its unreachable error family are deleted; `partial_window`, cursor pages, and terminal-owned bounds are the only fluent read windows. Production `EntityAuthority` construction is complete and internal, and hookless runtime semantics are gone. |
| 12 — Typed bootstrap and final residue | Complete (2026-07-14) | Generated `db()` / `core_db()` propagate `DatabaseBootstrapError` with the original typed `ic-memory` cause; string caching and panic fallback are deleted. Active docs, generated consumers, canisters, source guards, and benchmark labels use only current names. |
| 13 — Canonical schema mutation plan and diagnostics | Complete (2026-07-14) | `MutationPlan` is the sole schema plan from transition policy through concrete runners; generic rebuild/execution/capability shells are deleted, and transition metrics preserve the selected current plan kind. |
| 14 — Schema publication authority and rollback | Complete (2026-07-15) | Fake invalidation/publication phases, runtime-epoch identity, sink traits, production no-op adapters, manufactured visibility modes, fixed phase/progress copies, and test-only write/discard/rollback report shells are deleted. Staged validation plus actual store state own physical truth; the real publication gate is authoritative, and rejected field-path/expression additions roll back their physical attempt. |
| 15 — Accepted `SchemaInfo` authority | Complete (2026-07-15) | Production accepted schema views require their selected enum catalog and contain no generated field-kind bridge; catalog-less construction is explicitly test/model-only. |
| 16 — Cursor/admin public API hard cut | Complete (2026-07-15) | Raw cursor decoding is private, raw encoding is no longer a normal core API, and `AdminBatchRequest` has only the current first/next constructors. |
| 17 — Destructive DDL publication atomicity | Complete (2026-07-15) | `DROP COLUMN` and `DROP INDEX` carry exact physical before-images through accepted-schema publication, restore them on rejection, and no longer retain test-only rollback reports or accepted-before runner state in production. |
...
**Delete:** grouped eligibility/outcome/rejection types and properties, plus the
compile-fail fixture that names the retired observability type.
...
**Must not change:** reachable rejection meaning/text/JSON, policy decisions,
trusted SQL OFFSET behavior, or public result types. Unproduced diagnostic codes
are deleted and later raw values compacted under the pre-1.0 hard cut.
...
- [x] No deprecated public APIs.
- [x] No aliases for removed APIs.
- [x] No v1/v2 execution split; remaining version tags are documented durable or
      diagnostic schema identities.
- [x] No duplicate planner authority.
- [x] No duplicate admission authority.
- [x] No obsolete query DTOs.
- [x] No unreachable plan or route-observability variants.
- [x] No stale cursor format unless a documented persisted compatibility
      obligation requires it.
- [x] No old response pagination or projection-response facade.
- [x] No application-facing custom policy machinery or internal admission DTO
      exports.
- [x] No fallback route masking planner errors.
- [x] No tests preserving retired behavior.
- [x] No stale generated methods.
- [x] No active docs describing removed designs.
- [x] No metrics or diagnostics using obsolete terminology.
- [x] No compatibility module or alias without a documented live compatibility
      obligation.
- [x] Trusted/admin SQL entry is explicit at the method/type boundary and
      generated SQL remains controller-gated.
- [x] Diagnostics project typed planner/route truth without reparsing labels.
- [x] Accepted schema remains the sole runtime schema authority.
- [x] One current fail-closed decoder remains for each persisted format.
- [x] No generic mutation-result or projected-row response facade remains.
- [x] No fluent raw-limit or OFFSET construction path remains; partial windows
      and cursor pages name the public contract.
- [x] Production entity authority cannot be constructed without accepted
      schema and row-decode contracts.
- [x] No hookless runtime execution, recovery, reconciliation, or integrity
      behavior remains.
- [x] Generated database construction preserves typed memory-bootstrap failure
      and does not panic or stringify the cause.
- [x] Schema mutation plans represent executable current states only; no
      compatibility or unsupported pseudo-plan is carried through execution.
- [x] Schema mutation intent, publication preflight, and physical runner input
      share one canonical `MutationPlan`; no rebuild/execution-plan copy remains.
- [x] Schema transition diagnostics preserve the plan kind selected by
      reconciliation instead of relabelling current routes as exact matches.
- [x] Physical schema runners do not report accepted publication or runtime
      invalidation; `SchemaPublicationGate` is the sole accepted-snapshot owner.
- [x] Physical readiness comes from proof-bearing validation and actual
      index-store state, not parallel visibility, phase, or progress DTOs.
- [x] Failed field-path and expression-index publication rolls back the
      physical keys written by that attempt.
- [x] Production accepted `SchemaInfo` construction always carries accepted
      catalog authority and no generated field-kind bridge.
- [x] Raw cursor codecs and duplicate admin-batch continuation spellings are
      absent from the application-facing API.
- [x] Rejected `DROP COLUMN` and `DROP INDEX` publication restores exact
      accepted-before row and index state.
...
All 0.203 owner slices are complete:

| Owner boundary | Action | Target report/run |
| --- | --- | --- |
| Facade + generated SQL + admission | Slices 1–2 complete; fluent reads and diagnostic explain are the only policy consumers | Complete |
| Query planner + diagnostics | Slices 3–4 complete; typed access-choice facts and actual grouped execution mode/fallback reason are projected directly | Complete |
| Metrics + CLI + compile-fail suites | Slice 5 complete; generated-model fallback metrics, the CLI compatibility alias, and obsolete negative fixtures are deleted | Complete |
| Query facade + response facade | Slices 6–8 complete; fluent construction, `RowProjectionOutput`, canonical scalar attribution, and internal admission DTOs are singular | Complete |
| Contracts/docs | Slice 9 complete; current cursor, SQL, admission, durability, and grouped terminology match implementation | Complete |
| Response + schema mutation | Slices 10 and 13 complete; current direct results and one canonical executable mutation plan have no compatibility or capability-negotiation shells | Complete |
| Fluent + runtime authority | Slice 11 complete; named read windows, accepted entity authority, and registered runtime hooks are singular | Complete |
| Generated bootstrap + final docs | Slice 12 complete; generated sessions are fallible and typed, and active surfaces contain no stale names | Complete |

## Implementation Closeout Validation

| Validation area | Status | Evidence |
| --- | --- | --- |
| Formatting and patch hygiene | PASS | Workspace formatting and `git diff --check` pass after the final hard cut. |
| Changed-package compilation and lint | PASS | Changed crates pass warnings-denied all-target/all-feature Clippy; the core no-default-feature dependency build and focused facade/core/CLI checks also pass. |
| Query, planner, executor, admission, and schema tests | PASS | Focused intent, SQL lowering, pipeline, admission, read-admission, cursor, grouped-plan, route, metrics, projection, attribution, schema mutation, transition, and reconciliation suites pass. |
| Public/compile contracts | PASS | Facade trybuild passes both maintained public examples; generator and current UI/macro checks pass, with compatibility-only fixtures deleted. |
| Diagnostic contracts | PASS | Typed access-choice, grouped route, scalar attribution, compacted admission registry, CLI rendering, and EXPLAIN-focused checks pass. |
| Documentation and architecture invariants | PASS | Focused core/facade documentation builds; read-admission and durability invariant checks pass; active-source residue searches find none of the removed names. |
| Full repository test suite | NOT RUN | Repository rules reserve the full `make test` / push-workflow suite for the user. No full workspace test command was run. |
| Runtime performance benchmarks | NOT RUN | 0.203 removes ownership and public-surface sediment without changing intended query results or physical algorithms; no focused benchmark contract changed. |
| Representative raw Wasm | NOT RUN for 0.203.2 | The second-pass hard cut was not rebuilt for Wasm size. The earlier 0.203.0 measurement below is historical evidence only and is not claimed for the current worktree. |

## 0.203.0 Implementation Change and Cost Readout

- Changed inventory: **161 files** including the new maintained trusted-SQL
  compile example and detailed release notes; **2,082 insertions and 4,849
  deletions**, for an approximate net reduction of **2,767 lines**.
- Rust-only change: **141 files**, **1,605 insertions and 4,483 deletions**, for
  a net reduction of **2,878 Rust lines**.
- Implementation shape: **simpler**. Three owner modules and six
  compatibility-only UI fixtures are deleted; duplicate public stages,
  response/diagnostic aliases, inert policy facts, the test-only fluent OFFSET
  hook, and reconstructed route state disappear without replacement layers.
- Runtime performance: no benchmark delta is claimed because no physical
  execution algorithm was changed.
- Wasm size: representative raw non-gzipped Wasm is **556 bytes smaller**;
  gzip is secondary context at **67 bytes smaller**.

## 0.203.2 Second-Pass Change and Cost Readout

- Changed inventory: **193 files**, **2,556 insertions**, and **6,746
  deletions**, for a net reduction of **4,190 lines**. Rust accounts for
  **178 files**, **2,179 insertions**, and **6,563 deletions**, a net reduction
  of **4,384 Rust lines**.
- Implementation shape: **simpler**. Incomplete runtime entry paths, obsolete
  response shells, duplicate schema mutation plans, capability-negotiation
  scaffolding, compatibility pseudo-states, raw fluent limits, and stringly
  bootstrap failure disappear without replacement layers. Transition metrics
  now project the selected plan instead of reconstructing a coarse identity.
- Runtime performance: no benchmark delta is claimed because physical query
  algorithms were not intentionally changed.
- Wasm size: not measured for 0.203.2; raw non-gzipped Wasm remains the required
  primary measure before any size claim.
