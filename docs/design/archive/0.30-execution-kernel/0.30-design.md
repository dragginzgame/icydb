...
## 3.1 Frozen Layer Stack (0.30 Governance Lock)

For `0.30`, architectural audits and refactors must evaluate against this fixed stack:

```text
session
→ query (logical plan)
→ executor (route + planning + mutation planning)
→ access (eligibility + storage shape contracts)
→ index / data
→ commit (journal/apply/recovery primitives)
→ codec
```

This stack is a governance contract for `0.30`, not a suggestion.

Layering rules:

- Dependencies flow downward only.
- Lower layers must not import higher-layer types.
- `access` owns pushdown eligibility and storage-shape contracts consumed by executor/query; executor must not re-own those rules.
- `commit` is mechanical: marker/decode/apply/recovery primitives only. It must not own mutation planning.
- Storage-physical key encoding details belong below executor-facing contracts.

Audit enforcement:

- Every structural audit must start by restating this exact stack before findings.
- Findings must be classified against this stack (violation / no violation), not against ad-hoc reviewer preference.
- Cross-layer exceptions must be explicitly documented with owner, reason, and removal plan.
