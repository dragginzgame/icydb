...
* Optional helper: `MergeOrderedKeyStream` for two ordered key streams
...
* Wire `MergeOrderedKeyStream` into `Union`/`Intersection` execution yet
...
# 11. Post-Scope Addendum (Implemented Early)

One extra internal helper landed during 0.15 work:

```rust
let merged = MergeOrderedKeyStream::new(left, right);
```

This helper is a small overshoot beyond the strict 0.15 baseline. It is kept
internal and does not move pagination, cursor, filtering, or row loading logic
into the stream layer.

So the 0.15 non-relocation rule still holds.
