...
# `docs/design/archive/0.11-range-pushdown/0.11-design.md`

# 0.11 – Secondary Range Predicate Pushdown
...
for bounded secondary index traversal.

`KeyRange` remains primary-key-only access. 0.11 introduces `IndexRange` as the
dedicated secondary range access path.
