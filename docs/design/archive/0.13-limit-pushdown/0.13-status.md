...
This status tracks progress against `docs/design/archive/0.13-limit-pushdown/0.13-design.md` for
...
- [x] `0.13.0` released
- [x] `0.13.1` cleanup follow-up merged
...
Overall status for 0.13 tracked work: **100% complete**.
...
- Binary cursor work is intentionally decoupled to keep 0.13 constrained and
  shippable.
- Post-milestone cleanup also tightened internal layering:
  - pagination tests were de-duplicated to reduce maintenance cost before 0.14,
  - entity key-integrity decode was separated from codec policy (`db::entity_decode` vs `db::codec`).

---

## Next Checkpoint

1. Start 0.14 DESC planning and keep the existing 0.13 limit-pushdown coverage as the regression baseline.
2. Resolve the local `clippy` environment issue so
