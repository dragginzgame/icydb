# 0.23 — Decimal Consolidation & Internal Decimal Engine
...
* Scale metadata is preserved into runtime model as `FieldKind::Decimal { scale: Some(N) }` (transitional shape).
...
## 8️⃣ Runtime Surface Consolidation
...
## 9️⃣ Compatibility and Migration Strategy
