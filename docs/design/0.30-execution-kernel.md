...
- Landed in-tree: `ExecutionPlan` boundary, `ExecutionKernel` scaffold, kernel-owned residual retry, kernel-owned DISTINCT stream decoration, kernel-owned predicate compile policy, canonical `count_distinct_by` orchestration via `execute(...)`, shared aggregate streaming-input setup, shared aggregate fast-path fold helpers, shared aggregate materialized terminal helper, shared routed-stream aggregate fold helper, shared secondary-index probe/fallback fold helper, kernel-owned aggregate execution entrypoint delegation, kernel-owned aggregate reducer-dispatch adapter boundary, removal of duplicate legacy aggregate kernel orchestration, extraction of aggregate kernel orchestration into `kernel/aggregate.rs`, terminal-wrapper split out of `load/aggregate/mod.rs`, and dedicated aggregate contracts/orchestration/fast-path modules under `load/aggregate/`
- Remaining in 0.30: aggregate pipeline collapse and removal of `load/aggregate/mod.rs`
...
As of `0.30` in-tree progress, DISTINCT stream logic has been moved under `kernel/distinct.rs`.
...
- [landed] Extract aggregate kernel orchestration helpers into `kernel/aggregate.rs`
- [landed] Split aggregate terminal wrappers into `load/aggregate/terminals.rs`
- [landed] Split aggregate fast-path wiring into `load/aggregate/fast_path.rs`
- [landed] Split aggregate contracts and orchestration into `load/aggregate/contracts.rs` and `load/aggregate/orchestration.rs`
