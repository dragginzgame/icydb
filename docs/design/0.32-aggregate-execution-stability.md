...
## Theme
Unify aggregate execution mechanics.

## Scope
Still no public `GROUP BY` behavior.

## Dependency Chain
- Input from `0.31`: deterministic key/equality substrate and stable hash semantics.
- Output to `0.33`: planner/grouped execution can rely on one aggregate state engine and explicit execution-shape contracts.
