...
## 0.33.5 - 2026-02-28 - Boundary Authority and Compile Handoff Cleanup

### 📝 Summary

0.33.5 closes the remaining high-risk boundary drift in the query/executor stack by removing cross-domain semantic cycles, unifying aggregate taxonomy ownership, and isolating query planning from executor runtime representation.

### 🔧 Changed - Boundary Authority

- Removed the `contracts ↔ executor::group` semantic cycle by relocating grouped-key equality ownership to the grouped executor domain and keeping contracts focused on cross-domain value ordering semantics.
- Unified aggregate terminal taxonomy under one query-owned enum authority so query planning, explain projection, fingerprint hashing, route lowering, and aggregate execution consume one canonical aggregate-kind model.
- Removed parallel aggregate-kind projection and mapping paths in explain and route lowering, eliminating multi-owner aggregate interpretation drift between explain output, fingerprint tags, and runtime reducers.
- Moved query plan-shape policy ownership into `db::query::policy` and removed the prior `db::policy ↔ db::query` cross-module dependency loop.
- Removed grouped aggregate contract duplication by deleting executor-owned grouped-spec projection types and route-lowering bridges; grouped runtime now validates and consumes query-owned grouped aggregate specs directly.

### 🔧 Changed - Compile Handoff Isolation

- Introduced a query-owned `CompiledQuery` handoff and moved `Query::plan()` to return that handoff instead of executor runtime state.
- Moved compile-bridge ownership under executor and removed query-side runtime imports, so query planning contracts stay runtime-shape-agnostic while preserving existing session/fluent behavior.
- Tightened executor-plan visibility and pruned stale bridge helpers after the compile-handoff split, keeping runtime plan types crate-internal.

### 🔧 Changed - Query/Cursor Boundary Consolidation

- Removed cursor dependence on concrete `AccessPlannedQuery` by passing explicit access/order/page contracts into cursor preparation, revalidation, and continuation-derivation paths.
- Moved shared continuation-signature and cursor-plan error protocol types into `db::cursor_protocol` so query fingerprint/plan validation and cursor runtime consume one shared authority without query importing cursor internals.
- Removed query-plan-owned cursor-boundary reconstruction helpers and moved boundary derivation to the cursor domain, keeping cursor runtime semantics owned by cursor modules.

### 🔧 Changed - Commit Window Atomicity

- Replaced commit-window preflight mutate-and-rollback behavior with in-memory simulation so row/index stores are never mutated before commit marker persistence.
- Routed preflight commit preparation through overlay-aware row/index readers so sequential row-op validation remains accurate without opening a pre-marker panic/replay ambiguity window.

### 🧭 Migration Notes

- Public fluent/query APIs remain behavior-preserving; `plan()` now yields query-owned handoff state and executor conversion is explicit at the runtime boundary.
- No cursor token format or aggregate result-shape change is introduced by this patch.
- Internal cursor protocol ownership changed (`ContinuationSignature` and `CursorPlanError` now live under `db::cursor_protocol`) with behavior-preserving external query/cursor contracts.

### 🧪 Testing

- Re-ran `cargo check -p icydb-core` and `cargo test -p icydb-core --no-run` after boundary moves and visibility tightening.
- Re-ran focused query-intent and executor route/aggregate suites to confirm semantic parity across planning, routing, and grouped execution contracts.

---

## 0.33.4 - 2026-02-28 - Predicate Spine Convergence and Grouped Contract Hardening

### 📝 Summary

0.33.4 hardens grouped correctness boundaries and completes predicate-domain convergence under `db::predicate`, while preserving existing query/execution behavior at the public API surface.

### 🔧 Changed - Grouped Correctness Hardening
...
### 🔧 Changed - Predicate Domain Boundary
...
### 🔧 Changed - Access Canonicalization Authority
...
- Grouped continuation cursors are stricter against query-shape drift, and grouped resume with mismatched shape tokens is rejected earlier.
...
### 🔧 Changed - Load Terminal Dispatch
...
### 🔧 Changed - Grouped Vertical Slice Activation
...
### 🔧 Changed - Cursor and Resume Wiring
...
### 🔧 Changed - Route and Observability Alignment
...
### 🔧 Changed - Structural Compression
...
### 🔧 Changed - Stream Subsystem Ownership
...
## 0.33.1 - 2026-02-28 - Grouped Planning Follow-Through and Governance Realignment

### 📝 Summary

0.33.1 captures the main grouped-planning follow-through and boundary cleanup that landed immediately after the initial `0.33.0` tag, plus a broad audit/governance documentation realignment.

### 🔧 Changed - Grouped Planning and Execution Boundaries

- Completed grouped planning follow-through across query/executor handoff contracts, grouped validation paths, route planning hints, and explain/fingerprint grouped semantics.
- Kept grouped runtime explicitly gated while tightening grouped-plan transport and grouped aggregate-kind handling in route/executor boundaries.
- Moved active predicate runtime ownership from executor-local helpers into query-owned predicate runtime modules and kept index-predicate compile ownership under `db::index::predicate`.
- Normalized executor stream and index-predicate module ownership layouts to reduce boundary leakage and remove legacy placement seams.

### 🔧 Changed - Governance and Audit Structure

- Reorganized audit governance into explicit domain-owned recurring tracks (`access`, `contracts`, `crosscutting`, `cursor`, `executor`, `invariants`, `planner`, `storage`) plus oneoff reports.
- Migrated prior audit reports into the normalized audit hierarchy and aligned roadmap/design/status references to the new structure.
- Added the initial `0.33` detailed changelog surface under `docs/changelog` for line-level release tracking.

### 🧭 Migration Notes

- Public grouped runtime behavior remains unchanged in `0.33.1`; grouped execution enablement arrives in `0.33.2`.
- This patch line is primarily structural and governance-oriented, with grouped boundary hardening ahead of runtime activation.

### 🧪 Testing

- Updated grouped planner/route/explain/fingerprint and grouped validation test coverage to lock boundary behavior while grouped runtime remained gated.
...
### 🔧 Changed - Validation and Invariants
