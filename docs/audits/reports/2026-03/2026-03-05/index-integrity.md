# Index Integrity Audit - 2026-03-05
...
| Continuation anchor must remain within original envelope | `crates/icydb-core/src/db/index/scan.rs` + `db/index/envelope.rs` | PASS | Low |
...
| User/system key namespace enforced | `IndexKeyKind` checks in cursor anchor + reverse index paths | PASS | Low |
| Component arity mismatch rejected | `validate_index_range_anchor` in cursor anchor validation | PASS | Low |

## Mutation and Replay Symmetry

| Area | Evidence | Result | Risk |
| ---- | ---- | ---- | ---- |
| Preflight prepare before apply | `open_commit_window` in `db/executor/mutation/commit_window.rs` | PASS | Medium |
| Marker authority preserved on failure | `finish_commit` in `db/commit/guard.rs` | PASS | Low |
