...
| projection labels | `db/query/plan/expr/ast.rs`, `db/session/sql.rs`, `db/query/explain/plan.rs` | SQL projection + query builder projection | `145` projection-label hits across `19` files | Yes | Label rendering remains a post-plan presentation concern; demo_rpg/Pocket-IC test cleanup did not add a second label authority. |
