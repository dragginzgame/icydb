...
- `crates/icydb-core/src/db/executor/tests/pagination/*.rs`
  Size: 7857 lines across six files.
  Reason: heavy use of stale builder helpers and shared fixtures, plus deep cursor/ordering permutations.
  ...
  Recommendation: continue by owner slice. The small no-cursor limit-window contracts, the core primary-key cursor resume and fast-vs-fallback parity cases, and the first index-range pushdown/fallback parity slice are now live in `db/executor/tests/pagination.rs`; the remaining matrix backlog should still be revived deliberately, starting with deeper index-range anchor cases and composite-budget parity cases.
