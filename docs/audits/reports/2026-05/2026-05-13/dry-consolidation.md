# DRY Consolidation Audit - 2026-05-13
...
- compared baseline report path: `docs/audits/reports/2026-05/2026-05-07/dry-consolidation.md`
- rerun basis: dirty working tree at `2008c4809`; current local changes include `0.154.1` schema-runner startup execution and schema-default derive fixes
- code snapshot identifier: `2008c4809`
...
- comparability status: `comparable`; method, scope, and taxonomy match the 2026-05-07 baseline, but conclusions are dirty-tree qualified

## Evidence Artifacts

- `docs/audits/reports/2026-05/2026-05-13/artifacts/dry-consolidation/dry-consolidation-runtime-metrics.tsv`
- `docs/audits/reports/2026-05/2026-05-13/artifacts/dry-consolidation/dry-consolidation-pattern-counts.tsv`
- `docs/audits/reports/2026-05/2026-05-13/artifacts/dry-consolidation/dry-consolidation-module-pressure.tsv`
- `docs/audits/reports/2026-05/2026-05-13/artifacts/dry-consolidation/dry-consolidation-sql-decisions.tsv`
...
| ---- | ---- | ---- | ---- | ---- |
| baseline report path | `docs/audits/reports/2026-05/2026-05-04/dry-consolidation.md` | `docs/audits/reports/2026-05/2026-05-07/dry-consolidation.md` | one report forward | yes |
...
| snapshot state | dirty tree at `c18f3f8e5` | dirty tree at `2008c4809` | evidence is dirty-tree qualified | yes |
...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| schema-store vs runtime-store accepted snapshot reconciliation | `db/schema/reconcile.rs` | Accidental same-owner duplication introduced by startup execution wiring | yes | yes | partially | yes | high | medium-low | medium-low |
...
| schema mutation publication widened before runner execution | startup execution now calls `SchemaMutationRunnerInput::new`, `SchemaFieldPathIndexRunner::run`, and checks `physical_work_allows_publication()` before inserting `accepted_after` | closed for the single supported path | low-medium |
...
| ---- | ----: | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| schema reconciliation publication and metrics tail | 2 paths | schema-store metadata path and runtime-store startup path | no | yes | yes | partially | yes (`db/schema/reconcile.rs`) | high | low-medium | schema reconcile helper local to `reconcile.rs` | medium-low |
| field-path startup runner and staged publication contracts | 6 coordinated surfaces | reconcile startup adapter plus mutation field-path runner phases | no | yes | yes | yes | yes (`SchemaFieldPathIndexRunner`) | high | medium | keep runner contracts; only extract adapter helpers locally | low-medium |
...
| ---- | ----: | ----: | ---- | ---- | ---- | ---- |
| `crates/icydb-core/src/db/schema/reconcile.rs` | `503` runtime LOC | `2` | yes | startup bridge growth | runtime-store reconciliation repeats the metadata-only path's latest-snapshot, accepted-snapshot, footprint, and reconcile-metric tail while adding physical runner execution | medium-low |
...
| `crates/icydb-core/src/db/schema/mutation/field_path/publication.rs` | `605` runtime LOC | `1` | yes | publication phase reports | validation/invalidation/snapshot/physical publication reports repeat phase checks intentionally | low-medium |
...
| ---- | ---- | ---- | ---- | ---- | ----: | ---- | ---- | ---- |
| supported field-path schema mutation publication | `SchemaFieldPathIndexRunner` plus startup adapter in `db/schema/reconcile.rs` | transition admission; runner input binding; accepted row decode; staged physical validation; physical publication readiness; schema snapshot insert | defining plus staging plus startup application | yes | 6 | Safety-enhancing | low-medium | low-medium |
| accepted schema reconciliation publication/metrics tail | `db/schema/reconcile.rs` | schema-store path; runtime-store path | metadata-only and runtime-startup variants | partially | 2 | Same-owner duplication | medium-low | medium-low |
...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| field-path runner stages, validates, invalidates, publishes snapshot, and publishes physical store | field-path runner modules plus startup adapter | yes | yes | yes | each phase has a different failure/rollback boundary and should stay observable until supported multi-index publication exists | high |
...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ----: | ---- |
| accepted schema reconciliation tail helper | `db/schema/reconcile.rs` | Accidental same-owner duplication | yes | yes | partially | yes | safe local unification after 0.154.1 lands | schema reconcile boundary | low-medium | medium-low | 12-24 | medium-low |
...
| ---- | ---- | ---- | ---- | ---- |
| schema mutation runner phase contracts | staging, physical validation, runtime invalidation, snapshot handoff, and physical-store publication carry different rollback and visibility meanings | yes | do not merge phases | high |
| startup runtime-store reconciliation and metadata-only schema-store reconciliation | runtime-store path is allowed to execute physical runner work; metadata-only path must stay fail-closed for physical work | partially | only extract shared tail locally | medium-high |
...
| ---- | ---- | ---- | ---- | ---- |
| total duplication patterns found | `9` | `11` | `+2` | 0.154.1 startup schema mutation execution adds one same-owner reconcile duplication and one protective field-path runner phase family to the audit surface |
...
| same-layer accidental duplication count | `0` | `1` | `+1` | reconcile now has duplicated metadata/runtime accepted-snapshot tails that are eligible for later local helper extraction |
...
| boundary-protected duplication count | `7` | `8` | `+1` | field-path runner publication phases add a tracked protected duplication family |
| invariants with `>3` enforcement sites | `6` | `7` | `+1` | the field-path runner startup path adds a multi-site supported-publication invariant |
| error-construction families with `>3` custom mappings | `1` | `2` | `+1` | schema mutation startup runner errors are now worth tracking but remain typed and owner-local |
| drift surface reduction estimate | `medium-low` | `medium-low` | stable | current pressure is local reconcile/helper repetition, not divergent semantics |
| estimated LoC reduction range (conservative) | `10-20` | `22-44` | `+12-24` | the new reconcile helper candidate accounts for most added possible reduction |
...
| classified findings count | 39 | STEP 2A + STEP 4A + STEP 5A + STEP 6B + STEP 7B + STEP 8B rows requiring owner/safety judgment |
...
| boundary-protected findings count | 17 | rows where `Boundary-Protected? = yes` or `partially` across Steps 1A/4A/6B/8B |

## 1. Run Metadata + Comparability Note

- `DRY-1.2` method manifest applied; this run is method-comparable to the 2026-05-07 baseline.
- The scan used dirty working tree `2008c4809`, so conclusions should be treated as current-slice evidence rather than release-baseline evidence.
- The 0.154.1 schema-runner work is in scope because it changes startup runtime reconciliation and runner publication boundaries.

## 2. Mode A Summary: High-Impact Consolidation Opportunities

- No high-risk production DRY seam is open.
- SQL grouped/projection decision duplication remains closed: the audit helper reported `recomputed_decision_count=0` and `propagated_decision_count=0`.
- Supported schema mutation startup execution is not publishing from duplicated semantics: it consumes schema-owned runner input, accepted row contracts, and the field-path runner before inserting the accepted-after snapshot.

## 3. Mode A Summary: Medium Opportunities

- The new actionable item is local to `db/schema/reconcile.rs`: the metadata-only path and runtime-store path now duplicate accepted-snapshot publication and metrics tails.
- The safe fix is a local helper after this patch lands; it should not merge metadata-only reconciliation with runtime physical execution.

## 4. Mode A Summary: Low/Cosmetic Opportunities

- Route hint helpers still repeat small same-owner bounded-fetch plumbing.
- SQL projection/result shell repetition remains a small local cleanup candidate.
- Fluent non-paged terminal public wrappers remain local boilerplate.

## 5. Mode B Summary: Protective Redundancies (Keep Separate)

- Keep field-path runner publication phases split until rollback, invalidation, and multi-index physical publication are fully supported.
- Keep runtime-store startup execution separate from metadata-only schema-store reconciliation.
...
- Keep accepted schema runtime descriptors and generated-compatible proof construction separate from executor row decode consumption.

## 6. Dangerous Consolidations (Do Not Merge)

- Do not collapse the schema mutation runner phases into one untyped startup write path.
- Do not allow metadata-only reconciliation to execute physical schema mutation work.
...
- conservative LoC reduction: `22-44`
- DRY risk index: `4.0/10`

## 8. Analyst Verification Readout

- mechanical findings count: `11`
- classified findings count: `39`
- high-confidence candidate count: `3`
- boundary-protected findings count: `17`

## 9. Architectural Risk Summary

- Risk is still low-to-moderate and manageable.
- The only new consolidation pressure is local to schema reconciliation and comes from introducing the runtime-store startup path.
- The field-path runner phase repetition is protective and should remain split.

## 10. DRY Risk Index

`4.0/10`

Interpretation: low-to-moderate risk with no high-risk production consolidation seam open; new pressure is local to `db/schema/reconcile.rs`.
...
- `scripts/audit/runtime_metrics.sh docs/audits/reports/2026-05/2026-05-13/artifacts/dry-consolidation/dry-consolidation-runtime-metrics.tsv`
- `bash scripts/audit/sql_duplicate_decision_count.sh docs/audits/reports/2026-05/2026-05-13/artifacts/dry-consolidation/dry-consolidation-sql-decisions.tsv`
- `rg -n "execute_supported_field_path_index_addition|SchemaFieldPathIndexRunner::run|StructuralRowContract::from_accepted_schema_snapshot|SchemaMutationRunnerInput::new|supported_developer_physical_path|publication_preflight" crates/icydb-core/src/db/schema --glob '*.rs'`
- `rg -n "from_accepted_schema_snapshot|from_generated_compatible_accepted_decode_contract|generated_compatible_row_shape_for_model|AcceptedGeneratedCompatibleRowShape|row_decode_contract" crates/icydb-core/src/db --glob '*.rs' --glob '!**/tests/**'`
