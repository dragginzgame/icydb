...
- compared baseline report path: `docs/audits/reports/2026-05/2026-05-04/dry-consolidation.md`
- rerun basis: dirty working tree at `c18f3f8e5`; current local changes include accepted/generated-compatible row-decode authority work and 0.149 docs
- local exclusion note: save-path duplication is intentionally not assessed in this run because it is being handled in a parallel design/audit session
- code snapshot identifier: `c18f3f8e5`
...
- comparability status: `comparable`; method, scope, and taxonomy match the 2026-05-04 baseline, but conclusions are dirty-tree qualified

## Evidence Artifacts

- `docs/audits/reports/2026-05/2026-05-07/artifacts/dry-consolidation/dry-consolidation-pattern-counts.tsv`
- `docs/audits/reports/2026-05/2026-05-07/artifacts/dry-consolidation/dry-consolidation-module-pressure.tsv`
- `docs/audits/reports/2026-05/2026-05-07/artifacts/dry-consolidation/dry-consolidation-sql-decisions.tsv`
...
| ---- | ---- | ---- | ---- | ---- |
| baseline report path | `docs/audits/reports/2026-05/2026-05-03/dry-consolidation.md` | `docs/audits/reports/2026-05/2026-05-04/dry-consolidation.md` | one report forward | yes |
...
| snapshot state | clean tree at `13ec2bef4` | dirty tree at `c18f3f8e5` | evidence is dirty-tree qualified | yes |
...
| ---- | ---- | ---- | ---- |
| row/scalar projection evaluating planner expression shapes directly at runtime | executor and aggregate runtime paths still consume compiled expression contracts through reader adapters | closed in production | low |
...
| accepted row-decode contract attached without generated-compatible proof | `EntityAuthority::with_accepted_row_decode_contract` now requires `AcceptedGeneratedCompatibleRowShape`, and `RowLayout::from_generated_compatible_accepted_decode_contract` consumes the proof | closed in current dirty tree | low |
...
| ---- | ----: | ----: | ---- | ---- | ---- | ---- |
| `crates/icydb-core/src/db/schema/runtime.rs` | `486` | `2` | yes | descriptor proof authority | generated-compatible proof construction is centralized, but descriptor/field compatibility checks remain a growing authority surface | low-medium |
| `crates/icydb-core/src/db/executor/authority/entity.rs` | `180` | `1` | no | proof forwarding | entity authority now forwards an explicit generated-compatible proof into row layout construction | low |
| `crates/icydb-core/src/db/executor/terminal/row_decode/mod.rs` | `304` | `1` | no | proof-consuming bridge | row decode consumes accepted decode facts only with the generated-compatible row-shape proof | low |
| `crates/icydb-core/src/db/session/sql/execute/mod.rs` | `611` | `1` | yes | execution shell repetition | cached and bypass paths still repeat prepared-plan/projection-contract shells | low-medium |
| `crates/icydb-core/src/db/query/fluent/load/terminals.rs` | `778` | `1` | yes | partially-compressed | remaining repetition is public method shape and output conversion | low-medium |
...
| ---- | ---- | ---- | ---- | ---- |
| total duplication patterns found | `9` | `9` | `0` | save-path findings are deferred to the parallel session, leaving the comparable non-save pattern count stable |
| total high-risk divergence patterns | `0` | `0` | `0` | no duplicate evaluator, SQL shape decision, or generated-as-authority path reopened |
| same-layer accidental duplication count | `0` | `0` | `0` | same-owner findings are bridge/wrapper repetition, not accidental semantic duplication |
...
| boundary-protected duplication count | `7` | `7` | `0` | accepted row-decode proof propagation remains the tracked bridge family in this run |
| invariants with `>3` enforcement sites | `6` | `6` | `0` | save-path invariants are deferred to the parallel session |
...
| drift surface reduction estimate | `medium-low` | `medium-low` | stable | current pressure is local branch/bridge repetition, not divergent semantics |
| estimated LoC reduction range (conservative) | `18-36` | `10-20` | `-8-16` | save-path helper compression is out of scope for this run |
...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| none | n/a | n/a | n/a | n/a | n/a | production scalar, grouped, SQL shape, and accepted row-decode paths remain under explicit owner boundaries |
...
| classified findings count | 32 | STEP 2A + STEP 4A + STEP 5A + STEP 6B + STEP 7B + STEP 8B rows requiring owner/safety judgment |
...
- `DRY-1.2` method manifest applied; this run is method-comparable to the 2026-05-04 baseline.
- The scan used dirty working tree `c18f3f8e5`, so conclusions should be treated as current-slice evidence rather than release-baseline evidence.
- Save-path duplication is intentionally out of scope for this run because it is being handled in the parallel session.
...
- Accepted row decode is healthier than the May 4 baseline: generated-compatible proof construction is schema-owned, and row layout construction now consumes an explicit `AcceptedGeneratedCompatibleRowShape`.

## 3. Mode A Summary: Medium Opportunities

- No new medium-risk non-save consolidation candidate was found.
- The accepted row-decode proof bridge is boundary-sensitive, but the current shape has a clear schema-runtime owner and executor consumer.
...
- mechanical findings count: `9`
- classified findings count: `32`
- high-confidence candidate count: `2`
- boundary-protected findings count: `15`

## 9. Architectural Risk Summary

- Risk is low and manageable.
- The accepted/generated-compatible row-decode bridge is moving in the right direction because proof construction has a clear schema-runtime owner and executor code consumes a proof object.
- Save-path DRY pressure is deliberately excluded from this report and should be read from the parallel session's output instead.

## 10. DRY Risk Index

`3.6/10`

Interpretation: low risk with no high-risk non-save consolidation seam open.
...
- `scripts/audit/runtime_metrics.sh /tmp/icydb-dry-runtime-metrics.tsv`
- `bash scripts/audit/sql_duplicate_decision_count.sh /tmp/icydb-sql-duplicate-decision-count.tsv`
- `rg -n "from_accepted_decode_contract|from_generated_compatible_accepted_decode_contract|generated_compatible_row_shape_for_model|AcceptedGeneratedCompatibleRowShape|row_decode_contract" crates/icydb-core/src/db --glob '*.rs' --glob '!**/tests/**'`
