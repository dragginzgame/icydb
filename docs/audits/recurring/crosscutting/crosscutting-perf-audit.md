...
Calibration ordinal one builds the shared subject. Ordinals two and three must
name that successful workflow run, reuse its raw Wasm artifact, and verify the
same workflow, current source revision, and recorded raw Wasm SHA-256 before
measurement. This is same-source calibration reuse, not a previous-source
comparison path.
