...
- [x] Close remaining deterministic pre-commit validation gaps for delete/save planning so all fallible work is complete before commit-window open.  
  Owner: `Codex`
- [ ] Add final replay regressions for reverse-index correctness across mixed marker sequences (`save -> save -> delete`, retarget updates, and rollback path).  
  Owner: `Codex`
- [ ] Standardize error classification at RI boundaries (`Unsupported`, `Corruption`, `Internal`) and verify emitted classes in tests.  
  Owner: `Codex`
- [ ] Finalize operator-facing diagnostics for blocked deletes (include enough context to identify source entity/field quickly).  
  Owner: `Codex`
- [ ] Verify reverse-index and relation-validation metrics deltas are emitted consistently on success and failure paths.  
  Owner: `Codex`
