...
lane while keeping continuation movement explicit and atomic with successful
page processing. They return exactly one uncommitted page per call through
`LivePageStep`; decode that page, then commit the step before advancing again.
A failed decode leaves caller-owned traversal state unchanged. They
intentionally do not collect a complete traversal.
