...
| Atomic single-entity batches | Admit and stage every heterogeneous item before opening one commit window. Duplicate targets across operation kinds reject. Unique, relation, index, and row constraints consume the complete final overlay; public result conversion and response bounds finish before marker publication. One rejected item rejects the entire batch and consumes no Identity. |
