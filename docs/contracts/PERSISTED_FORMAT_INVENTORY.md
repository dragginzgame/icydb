...
| --- | --- | --- | --- |
| Stable-memory allocation roles | `crates/icydb-core/src/traits/mod.rs`, `crates/icydb-model/src/node/canister.rs`, `crates/icydb-model-legacy/src/node/canister.rs`, `crates/icydb-schema-derive/src/node/canister.rs` | Assigns the commit and integrity-progress memory domains plus each journaled store's data, index, schema, and journal memories. `icydb-model` is the final authoring owner; the legacy/compiler paths remain only until the 0.213 consumer cutover. | Stable identity contract; every role is collision-checked and remapping is outside the durability guarantee. |
...
| Schema-application records | `crates/icydb-core/src/db/schema/application_receipt.rs`, `crates/icydb-core/src/db/schema/application_store.rs` | Database-wide proposal idempotency, terminal/pending receipts, compact failure identity, and the exact accepted-constraint activations owned by a pending schema-change job. | One current version-1 checksummed record envelope in a restricted database-control BTreeMap region. Records are capped at 64 KiB, pending jobs at 512 activation identities, and the store at 64 retained submissions; impossible pending/terminal activation closure, noncanonical bytes, key/receipt mismatch, checksum failure, and compare-and-replace drift fail closed. No prior record shape, alias, or fallback exists. |
