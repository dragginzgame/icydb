...
| Commit marker payload | `crates/icydb-core/src/db/commit/marker.rs`, `crates/icydb-core/src/db/commit/store/marker_envelope.rs` | Marker-authorized row mutation and recovery authority. | Versioned marker envelope and bounded payload; unsupported versions fail closed. |
...
| Raw row envelopes | `crates/icydb-core/src/db/codec/mod.rs`, `crates/icydb-core/src/db/data/row.rs`, `crates/icydb-core/src/db/data/structural_row.rs` | Canonical row payload bytes in stable data stores. | Row envelope is versioned and bounded before trusted row construction. |
...
| Value-storage payloads | `crates/icydb-core/src/db/data/structural_field/value_storage/` | Recursive `Value` storage lane for fields that cannot use scalar fast paths. | Local extension tags, bounded recursion, bounded allocation, and fallible decode. |
| Accepted schema snapshots | `crates/icydb-core/src/db/schema/codec.rs`, `crates/icydb-core/src/db/schema/snapshot.rs`, `crates/icydb-core/src/db/schema/store.rs` | Runtime schema authority for row layout, field contracts, indexes, and relations. | Versioned Candid wire container; generated models are not runtime authority. |
