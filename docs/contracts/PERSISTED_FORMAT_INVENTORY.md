...
| Accepted schema snapshots | `crates/icydb-core/src/db/schema/codec.rs`, `crates/icydb-core/src/db/schema/snapshot.rs`, `crates/icydb-core/src/db/schema/source_binding.rs`, `crates/icydb-core/src/db/schema/store.rs`, `crates/icydb-core/src/db/schema/composite_catalog.rs` | Runtime schema authority for row layout, exact field contracts, nominal enum/composite catalogs, immutable source-to-accepted identity bindings, logical structural identities, indexes, relations, and the structural constraint registry. | One current version-1 Candid wire with the required `ICYZ` contract profile; the canonical codec rejects payloads above 512 KiB before decode or emission, and current accepted bundle/root, enum catalog, and source-binding envelopes retain hard-cut magic identities and version 1. Source bindings are capped at 2 MiB, canonically ordered, one-to-one within each structural namespace, closed over the same accepted bundle, and included in its semantic fingerprint. Every entity carries the accepted constraint catalog, live activation records, planner-invisible candidate index and relation owners, and non-reusing allocator, plus current/history-floor layout versions, stable logical index and relation IDs, bounded canonical accepted check expressions, and per-field introduction layout, future insert-default, and frozen historical-fill facts. These identities are fingerprinted contract facts; non-current forms do not decode, and generated models are proposal-only rather than runtime authority. Dense physical index ordinals and isolated candidate index/relation generations remain separate from logical identity. The 0.213.1 source-binding addition is a pre-1.0 hard cut: development stores written by 0.213.0 must be recreated, with no fallback decoder or translation path. |
