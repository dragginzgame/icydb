...
| Accepted schema snapshots | `crates/icydb-core/src/db/schema/codec.rs`, `crates/icydb-core/src/db/schema/snapshot.rs`, `crates/icydb-core/src/db/schema/store.rs`, `crates/icydb-core/src/db/schema/composite_catalog.rs` | Runtime schema authority for row layout, exact field contracts, nominal enum/composite catalogs, logical structural identities, indexes, and relations. | One current version-1 Candid wire with the required `ICYU` contract profile; current accepted bundle/root and enum catalog envelopes retain their hard-cut magic identities and version 1. Every entity carries constraint-ID high-water state, current/history-floor layout versions, stable logical index and relation IDs, and per-field introduction layout, future insert-default, and frozen historical-fill facts. These identities are fingerprinted contract facts; pre-`ICYU` forms do not decode, and generated models are not runtime authority. Dense physical index ordinals remain separate from logical index identity. |
