...
| --- | --- | --- | --- |
| Stable-memory allocation roles | `crates/icydb-core/src/traits/mod.rs`, `crates/icydb-schema/src/node/canister.rs`, `crates/icydb-schema-derive/src/node/canister.rs` | Assigns the commit and integrity-progress memory domains plus each journaled store's data, index, schema, and journal memories. | Stable identity contract; every role is collision-checked and remapping is outside the durability guarantee. |
