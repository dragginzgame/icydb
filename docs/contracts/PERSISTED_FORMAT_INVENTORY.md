...
| --- | --- | --- | --- |
| Stable-memory allocation roles | `crates/icydb-core/src/traits/mod.rs`, `crates/icydb-model-legacy/src/node/canister.rs`, `crates/icydb-schema-derive/src/node/canister.rs` | Assigns the commit and integrity-progress memory domains plus each journaled store's data, index, schema, and journal memories. The model path is temporary during the 0.213 package cutover. | Stable identity contract; every role is collision-checked and remapping is outside the durability guarantee. |
