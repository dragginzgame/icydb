...
| Integrity inspection progress | `crates/icydb-core/src/db/integrity/job.rs`, `crates/icydb-core/src/db/integrity/progress_store.rs` | Separately persists bounded Deep checkpoints, proof identity, replayable receipts, pending terminal intent, terminal acknowledgement, and retention state without mutating inspected authority. | One current version-1 allocation header and one checksummed current-form job envelope. Records are capped at 512 KiB with frozen global/per-owner counts. Unknown versions, malformed headers, corrupt checksums, impossible state/receipt pairs, invalid proof/checkpoint closure, and wrong record identities fail closed; no prior format decoder or fallback exists. The fair retention scan cursor is an advisory heap-only scheduling hint: restart safely resumes at the first key, and no cursor byte enters this format or proof authority. |
