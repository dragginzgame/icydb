...
- [ ] Full ASC/DESC matrix closure for offset + continuation across fast-path and fallback shapes
- [ ] Full regression sweep re-run and green

Overall status: **in progress; protocol/runtime offset semantics are implemented, matrix closure and release verification remain**.
...
## 0.21 Plan (Remaining Work)

1. Matrix closure for cursor+offset semantics:
- Add/finish ASC + DESC coverage across PK, secondary pushdown, index-range, and fallback/composite paths.
- Include resume-chain assertions (`offset` applied once, no duplicate rows, strict suffix on continuation).

2. DISTINCT parity with cursor+offset:
- Add/finish DISTINCT + offset continuation parity cases (fast-path vs fallback where both are valid).

3. Release prep:
- Final pass on docs/changelog wording for 0.21 user-facing semantics.
- Re-run full regression sweep, then cut `0.21.0`.
...
- 0.21 changelog entry reflects shipped behavior (no speculative items).
