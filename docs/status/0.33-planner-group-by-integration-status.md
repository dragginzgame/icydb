...
## 0.33.2 Stress Focus (Next Few Days)

Track these checks continuously while `0.33.2` hardening is active:

- Resume correctness parity: full run must equal paginated+resume run across page sizes, group cardinalities, filtered groups, and empty-result cases.
- Deterministic group order: grouped output order must remain stable and deterministic across randomized insertion order/entity IDs and across resume boundaries.
- Memory budget enforcement: high-cardinality and large-key/state runs must enforce limits with typed bounded failure (no panic or runaway heap behavior).
- Predicate interaction: `WHERE + GROUP BY + pagination` must preserve row-level filtering invariants so filtered rows never mutate grouped aggregate state.
