...
- [x] Aggregate fast-path wiring split into `load/aggregate/fast_path.rs`
...
- [ ] `load/aggregate/mod.rs` removed
- [ ] Route planner decomposition completed (`capability/mode/fast_path/hints`)
- [ ] Hard-mode parity + performance gates green
- [x] Changelog entry added for `0.30.0`

Current `0.30.0` status: **in progress (phase 1 and phase 2 complete; phase 3 underway)**.
...
10. Aggregate fast-path folding convergence:
   - `crates/icydb-core/src/db/executor/load/aggregate/fast_path.rs`
...
19. Aggregate fast-path module split:
   - `crates/icydb-core/src/db/executor/load/aggregate/fast_path.rs`
   - (verified-route gate, fast-path dispatch, and shared fast-path fold wiring moved from root aggregate module)
...
- [ ] Remove remaining duplicate aggregate orchestration branches and special dispatch
- [x] Remove `count_distinct_by` unique execution path
- [ ] Keep all aggregate/pagination parity tests unchanged and green

## Phase 4 - Structural Cleanup

- [ ] Remove `load/aggregate/mod.rs` by converging to kernel-owned reducers
- [ ] Split `route/planner.rs` into planned submodules without behavior changes
- [ ] Keep route capability derivation and access-shape boundaries route-owned

## Phase 5 - Parity + Performance Gates

- [ ] Continuation token/signature bytes parity lock (golden tests)
- [ ] Strict-vs-subset compile route-decision log parity lock (golden tests)
- [ ] Route trace output parity lock
- [ ] No >5% regression on common scan benchmarks
- [ ] No new hot-path `Vec` allocation churn introduced
...
- [ ] Cursor continuation/signature bytes identical before/after (unchanged query shapes)
- [ ] Route trace output identical before/after (unchanged plans)
- [ ] Benchmark shows no >5% regression on common scans
- [ ] No new `Vec` allocations in documented hot paths
- [ ] Golden tests: continuation token equality
- [ ] Golden tests: strict/subset compile route-decision logs
...
5. `load/aggregate/mod.rs` is deleted
...
Executed for current 0.30 phase-2/3 slice:
...
- full project gate: `make fmt-check && make clippy && make check && make test`
- benchmark + allocation audit for common scan paths
