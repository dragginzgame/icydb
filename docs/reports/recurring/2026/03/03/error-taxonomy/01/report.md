# Strict Error Taxonomy Audit - 2026-03-03
...
| `CursorPlanError`, `CursorDecodeError`, `TokenWireError` | `crates/icydb-core/src/db/cursor/error.rs`, `crates/icydb-core/src/db/codec/cursor.rs`, `crates/icydb-core/src/db/cursor/token/error.rs` | Cursor decode vs payload invariants separated |
...
| Cursor plan failure -> runtime internal error | `InternalError::from_cursor_plan_error` in `error.rs` | Yes (mapped to cursor invariant with preserved semantics) | Medium |
| Access plan validation -> runtime internal error | `InternalError::from_executor_access_plan_error` in `error.rs` | Yes (query invariant) | Low-Medium |
...
| Cursor malformed payload stays cursor-domain invalid input/invariant | `CursorPlanError` variants and token decode paths | PASS | Medium |
...
- Most drift-sensitive edge remains cursor plan/payload mapping, because it bridges decode, semantic validation, and executor revalidation.
