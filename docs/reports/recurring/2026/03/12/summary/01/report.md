# Recurring Audit Summary - 2026-03-12

## Report Preamble

- scope: run of all recurring audit definitions under `docs/audits/recurring/`
- compared baseline report path: `docs/audits/reports/2026-03/2026-03-10/summary-3.md`
...
2. `contracts/error-taxonomy` -> `error-taxonomy.md` (Risk: 4/10)
3. `contracts/resource-model-compliance` -> `resource-model-compliance.md` (PASS=3, PARTIAL=0, FAIL=0)
...
6. `crosscutting/crosscutting-layer-violation` -> `layer-violation.md` (Risk: 4/10)
...
9. `executor/cursor-ordering` -> `cursor-ordering.md` (Risk: 3/10)
10. `executor/executor-state-machine-integrity` -> `state-machine-integrity.md` (Risk: 4/10)
11. `integrity/invariant-preservation` -> `invariant-preservation.md` (Risk: 4/10)
12. `range/boundary-envelope-semantics` -> `boundary-semantics.md` (Risk: 3/10)
